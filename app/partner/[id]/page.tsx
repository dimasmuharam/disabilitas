...
import { 
  ...
  Award, BookOpen, CheckCircle2, ArrowRight,
  Zap, Calendar, LayoutDashboard, Share2
} from "lucide-react";
...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description: partner.description || `Melihat kontribusi dan komitmen inklusi ${partner.name} dalam memberdayakan talenta disabilitas di Indonesia.`,
    alternates: {
      canonical: `https://disabilitas.com/p/${params.id}`,
    },
  };
}

export default async function PartnerPublicProfile({ params }: { params: { id: string } }) {
  // 1. Fetch Data Partner
  const { data: partner } = await supabase
    .from("partners")
    .select("*")
    .eq("id", params.id)
  ...
  // 2. Fetch Data Statistik Talenta (Triple Table Sync)
  ...
  // Ambil data talenta untuk statistik visual
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-900">
      {/* HERO SECTION - Prestigius & Bold */}
      <div className="bg-slate-900 pt-32 pb-20 px-4 text-white">
        <div className="mx-auto max-w-5xl">
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
            <div className="space-y-4">
              <span className="inline-block rounded-full bg-blue-600 px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.2em]">
              ...
              <div className="flex flex-wrap items-center gap-4 text-slate-400">
                ...
                {partner.website && (
                  <a href={partner.website} target="_blank" className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest hover:text-white transition-colors">
            ...
            <div className="rounded-[2.5rem] bg-white/10 p-8 backdrop-blur-md border border-white/10 text-left min-w-[240px]">
              ...
              <p className="mt-4 text-[9px] font-bold uppercase text-emerald-400 leading-tight">
                Telah Terpeta Melalui Riset Nasional disabilitas.com
              </p>
            </div>
          </div>
        </div>
      </div>{/* IMPACT & NARRATIVE SECTION */}
      <div className="mx-auto -mt-10 max-w-5xl px-4">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="rounded-[3rem] border-2 border-slate-100 bg-white p-10 text-left shadow-xl shadow-slate-200/50">
              ...
              <div className="space-y-6 text-xl font-medium leading-relaxed text-slate-800 md:text-2xl italic">
                ...
                <p>
                  Melalui berbagai inisiatif pengembangan kompetensi, institusi ini mencatatkan tingkat keterserapan kerja alumni sebesar <strong>{employed > 0 ? Math.round((employed/totalImpact)*100) : 0}%</strong>. 
                  Ini adalah manifestasi nyata dari visi kemandirian ekonomi bagi warga negara disabilitas.
                </p>
                <p className="text-sm not-italic text-slate-500 leading-relaxed border-t border-slate-50 pt-6">
                  {partner.description || "Institusi ini sedang dalam proses memperbarui pernyataan komitmen inklusinya untuk riset nasional disabilitas.com."}
                </p>
              </div>
            </section>
            ...
            <section className="space-y-6">
              ...
              <div className="space-y-4">
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-4 text-[10px] font-bold text-slate-400 uppercase">
                        ...
                        <span className="flex items-center gap-1"><MapPin size={12} /> {t.is_online ? "Online" : t.location}</span>
                      </div>
                    </div>
                ...
                {archivedTrainings.map((t) => (
                  <div 
                    key={t.id} 
                    className="flex flex-col md:flex-row items-start md:items-center justify-between rounded-[2.5rem] border-2 border-slate-50 bg-slate-50/50 p-8 grayscale opacity-70"
                  >
                    <div className="space-y-2">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Batch Terakhir: {new Date(t.end_date).getFullYear()}</p>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-8">
            {/* Visual Chart - Disability Types */}
            <div className="rounded-[3rem] border-2 border-slate-100 bg-white p-8 text-left shadow-lg">
                  ...
                  <div key={type} className="space-y-2">
                    ...
                    <div className="h-3 w-full overflow-hidden rounded-full bg-slate-50">
                      <div 
                        ...
                        style={{ width: `${(count / totalImpact) * 100}%` }} 
                      />
                    </div>
                  </div>
                ))}
                {totalImpact === 0 && <p className="text-[10px] italic text-slate-300">Data riset sedang diproses...</p>}
              </div>
            </div>

            {/* Facilities / Accommodations Badge Grid */}
            <div className="rounded-[3rem] bg-slate-900 p-8 text-white shadow-2xl">
              <h3 className="mb-6 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-400">
                <ShieldCheck size={16} /> Akomodasi Tersedia
              </h3>
              <div className="flex flex-wrap gap-2">
                {partner.master_accommodations_provided?.map((item: string) => (
                  <span key={item} className="rounded-xl bg-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-tight text-slate-300 border border-white/5">
                    {item}
                  </span>
                ))}
              </div>
              <div className="mt-8 border-t border-white/10 pt-6">
                <p className="text-[10px] font-medium leading-relaxed italic opacity-60">
                  Data akomodasi diverifikasi mandiri oleh institusi untuk transparansi layanan publik bagi disabilitas.
                </p>
              </div>
            </div>
