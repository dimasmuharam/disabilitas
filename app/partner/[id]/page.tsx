      ...
      <div className="flex min-h-screen items-center justify-center p-10 text-center font-sans">
        <div className="max-w-md space-y-4">
          ...
          <p className="text-xs font-bold uppercase tracking-widest text-slate-400">
            Data institusi belum tersedia secara publik.
          </p>
          <Link href="/" className="mt-6 inline-block rounded-full bg-slate-900 px-8 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-xl transition-all hover:bg-blue-600">
            Kembali
          </Link>
        </div>
      </div>
            ...
            <div className="flex-1 space-y-4 text-center lg:text-left">
              <div className="space-y-2 text-left">
                ...
                <div className="flex flex-wrap gap-6 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                  <span className="flex items-center gap-2 italic"><BookOpen className="text-blue-600" size={16} /> {partner.category || "Institusi"}</span>
                  <span className="flex items-center gap-2 italic"><MapPin className="text-red-600" size={16} /> {partner.location || "Indonesia"}</span>
                </div>
              </div>
            </div>
              ...
              <div className="mb-4 flex items-center gap-4 text-left">
                ...
                <div className="flex-1">
                  <div className="mb-1 flex items-end justify-between">
                    <h3 className="text-sm font-black uppercase tracking-tighter">{badgeConfig.label}</h3>
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full border border-current/10 bg-white/50">
                    <div className={`h-full ${badgeConfig.pColor} transition-all duration-1000`} style={{ width: `${score}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-20 lg:col-span-2">
            <section className="space-y-6">
              <h2 className="flex items-center gap-3 text-2xl font-black uppercase italic tracking-tighter text-slate-900">
                <Info className="text-blue-600" size={28} /> Visi Inklusi
              </h2>
              <div className="rounded-[3rem] border-2 border-slate-100 bg-white p-10 italic shadow-sm">
                <p className="whitespace-pre-line text-lg font-medium leading-relaxed text-slate-700">
                  {partner.description || "Berkomitmen penuh mendukung pemberdayaan talenta disabilitas."}
                </p>
              </div>
            </section>
            <section className="space-y-8">
              <h2 className="flex items-center gap-3 border-b-4 border-blue-600 pb-4 text-2xl font-black uppercase italic tracking-tighter text-slate-900">
                <Zap className="fill-amber-500 text-amber-500" size={32} /> Program Pelatihan
              </h2>
              <div className="grid gap-6">
                {(trainings || []).length > 0 ? trainings?.map((t) => (
                  <Link key={t.id} href={`/pelatihan/${t.id}`} className="group flex flex-col items-center justify-between gap-8 rounded-[3.5rem] border-2 border-slate-100 bg-white p-8 shadow-sm transition-all hover:border-slate-900 hover:shadow-2xl md:flex-row">
                    ...
                    <div className="rounded-3xl bg-slate-900 p-5 text-white shadow-xl transition-all group-hover:bg-blue-600 group-hover:translate-x-2">
                      <ArrowRight size={24} />
                    </div>
                  </Link>
                )) : (
                  <div className="rounded-[4rem] border-4 border-dashed border-slate-100 bg-slate-50 p-20 text-center font-black uppercase italic tracking-[0.2em] text-slate-400 opacity-50">
          ...
          <aside className="space-y-12">
              ...
              <div className="relative z-10 space-y-10">
                <div className="flex items-end justify-between border-l-4 border-blue-600 pl-6">
                  <div>
                    ...
                    <p className="mt-2 text-[8px] font-black uppercase italic tracking-tighter text-slate-400">{isUni ? "Mahasiswa & Alumni" : "Peserta"} Terpeta</p>
                  </div>
                  <div className="text-right">
                    <TrendingUp className="mb-1 ml-auto text-emerald-400" size={24} />
            ...
            <section className="space-y-8 rounded-[3.5rem] border-2 border-slate-900 bg-white p-10 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)]">
              <h3 className="flex items-center gap-3 text-lg font-black uppercase tracking-tighter text-slate-900">
                <Users className="text-blue-600" size={24} /> Spektrum Inklusi
              </h3>
                ...
                {Object.entries(disabilityDist).map(([type, count]) => (
                  <div key={type} className="space-y-2">
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-slate-50">
                      <div className="h-full bg-gradient-to-r from-blue-600 to-purple-600" style={{ width: `${totalImpact > 0 ? (count / totalImpact) * 100 : 0}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
