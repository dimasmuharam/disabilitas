...
export default async function PublicPartnerProfile({ params }: { params: { id: string } }) {
  // QUERY TUNGGAL: Sangat ringan, semua statistik sudah ada di sini
  ...
  // Data dari Materialized Stats (Hasil SQL tadi)
  const totalTalenta = (partner.stats_academic_total || 0) + (partner.stats_impact_total || 0);
  const totalHired = (partner.stats_academic_hired || 0) + (partner.stats_impact_hired || 0);
            ...
            <div className="flex-1 space-y-4 text-center lg:text-left">
              <div className="space-y-2 text-left">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="flex items-center gap-1 rounded-full bg-blue-600 px-4 py-1.5 text-[10px] font-black uppercase italic text-white shadow-lg"><CheckCircle2 size={14} /> Verified Partner</span>
                </div>
            ...
            <div className={`max-w-xs rounded-[2.5rem] border-2 p-6 shadow-sm md:max-w-sm ${badge.color} text-left`}>
              <div className="mb-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="mb-1 flex items-end justify-between"><h3 className="text-sm font-black uppercase tracking-tighter">{badge.label}</h3><span className="text-xs font-black italic">{score}%</span></div>
          ...
          <div className="lg:col-span-2 space-y-20">
            ...
            <section className="space-y-8">
              <div className="flex items-center justify-between border-b-4 border-blue-600 pb-4">
                <h2 className="flex items-center gap-3 text-2xl font-black uppercase italic tracking-tighter"><Zap className="fill-amber-500 text-amber-500" size={32} /> Program Pelatihan</h2>
              </div>
              <div className="grid gap-6">
                {(trainings || []).length > 0 ? trainings.map((t) => (
                  <Link key={t.id} href={`/pelatihan/${t.id}`} className="group flex flex-col items-center justify-between gap-8 rounded-[3.5rem] border-2 border-slate-100 bg-white p-8 shadow-sm transition-all hover:border-slate-900 hover:shadow-2xl md:flex-row">
                    <div className="flex-1 text-left">
                      ...
                      <div className="mt-2 flex items-center gap-4 text-[10px] font-black uppercase text-slate-400 italic"><span>Batch: {new Date(t.start_date).getFullYear()}</span><span className="text-blue-600">Open Registration</span></div>
                    </div>
                ...
                )) : <div className="rounded-[4rem] border-4 border-dashed border-slate-100 bg-slate-50 p-20 text-center font-black uppercase italic tracking-[0.2em] text-slate-400 opacity-50 text-left">Belum ada program pelatihan.</div>}
              </div>
            </section>
          </div>
            ...
            <section className="relative overflow-hidden rounded-[3.5rem] bg-slate-900 p-10 text-white shadow-2xl text-left">
              ...
              <div className="relative z-10 space-y-10">
                <div className="flex items-end justify-between border-l-4 border-blue-600 pl-6">
                  <div><p className="text-3xl font-black italic text-white leading-none">{totalTalenta} Jiwa</p><p className="mt-2 text-[8px] font-black uppercase text-slate-400">{isUni ? "Alumni & Mahasiswa" : "Peserta Pelatihan"}</p></div>
                  <div className="text-right">
                    <TrendingUp size={24} className="mb-1 ml-auto text-emerald-400" />
                    <p className="text-xl font-black text-emerald-400 leading-none">{employabilityRate}%</p>
                    <span className="text-[7px] font-black uppercase text-slate-500 tracking-tighter">Terserap Kerja</span>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                {Object.entries(disMap).map(([type, count]) => (
                  <div key={type} className="space-y-2">
                    <div className="flex justify-between text-[9px] font-black uppercase text-slate-600"><span>{type}</span><span>{Math.round((Number(count)/totalTalenta)*100)}%</span></div>
                    <div className="h-2 w-full overflow-hidden rounded-full bg-slate-50"><div className="h-full bg-gradient-to-r from-blue-600 to-purple-600" style={{ width: `${(Number(count)/totalTalenta)*100}%` }}></div></div>
                  </div>
                ))}
              </div>
            </section>
