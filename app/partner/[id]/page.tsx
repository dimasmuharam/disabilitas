...
export default async function PublicPartnerProfile({ params }: { params: { id: string } }) {
  // 1. Ambil data partner langsung dari tabel partners (termasuk inclusion_score)
    ...
    return (
      <div className="flex min-h-screen items-center justify-center p-10 text-center font-sans">
        <div className="max-w-md space-y-4">
          <div className="mx-auto size-20 rounded-full bg-slate-100 flex items-center justify-center text-slate-400"><Info size={40} /></div>
          ...
          <Link href="/" className="inline-block mt-6 rounded-full bg-slate-900 px-8 py-3 text-[10px] font-black uppercase tracking-widest text-white">Kembali</Link>
        </div>
      </div>
    );
  }

  // 2. Fetch Data Riset Talenta (Sertifikasi + Pendidikan)
  ...
  // 3. Ambil data inclusion_score yang sudah tersedia di database
  ...
  // Konfigurasi Badge berdasarkan Skor Database
      ...
      <header className="border-b-2 border-slate-100 bg-white shadow-sm">
        <div className="mx-auto max-w-7xl px-6 py-16">
          <div className="flex flex-col items-center gap-10 lg:flex-row lg:items-end">
            ...
            {/* INCLUSION SCORE LANGSUNG DARI DATABASE */}
      ...
      <main className="mx-auto max-w-7xl px-6 py-16 text-left">
        <div className="grid gap-16 lg:grid-cols-3">
          ...
          <aside className="space-y-12">
            ...
            <button className="flex w-full items-center justify-center gap-3 rounded-[2rem] border-2 border-slate-200 bg-white py-5 text-[10px] font-black uppercase tracking-widest text-slate-900 transition-all hover:border-slate-900"
