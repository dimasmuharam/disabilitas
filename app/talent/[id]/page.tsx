...
// SINKRONISASI METADATA & CANONICAL
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  const profile = await getTalentProfile(params.id)
  if (!profile) return {}
  return {
    title: `${profile.full_name} | Talenta Inklusif disabilitas.com`,
    description: `Profil profesional ${profile.full_name}, talenta ${profile.disability_type} di disabilitas.com`,
    alternates: {
      ...
      <div className="bg-slate-900 text-white pt-24 pb-36 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center relative z-10">
          ...
          <h1 className="text-4xl font-black tracking-tighter uppercase mb-2 leading-none flex items-center gap-3">
            {profile.full_name}
            {/* SINKRONISASI DENGAN ADMIN VERIFICATION */}
            {profile.is_verified && (
              <ShieldCheck size={32} className="text-blue-400 fill-blue-400/20" title="Terverifikasi Admin"/>
            )}
          </h1>
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-24 relative z-20 space-y-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-8">
            ...
            <section className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800">
              ...
              <div className="relative pl-6 border-l-4 border-purple-100 dark:border-slate-800">
                ...
                {/* Menampilkan Major (Jurusan) */}
                <p className="text-blue-600 font-bold uppercase text-xs mt-1">{profile.major || "General Studies"}</p>
                <p className="text-xs text-slate-500 font-bold mt-3 italic">{"Model Pendidikan: "}{profile.education_model}</p>
              </div>
            </section>
          </div>
          ...
          <div className="space-y-6">
            ...
            {/* PENYESUAIAN KERJA - SINKRON ARRAY */}
            <section className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-xs font-bold leading-relaxed">
                    {Array.isArray(profile.used_assistive_tools) ? profile.used_assistive_tools.join(", ") : "-"}
                  </p>
                </div>
                <div>
