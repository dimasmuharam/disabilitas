...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description: `Hubungi ${profile.full_name}, profesional berbakat dengan ragam ${profile.disability_type} di disabilitas.com. Lihat rekam jejak dan keahliannya.`,
    alternates: {
...
async function getTalentProfile(id: string) {
  // Mengambil data profil beserta relasi tabel work_experiences dan certifications
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
      ...
      <div className="bg-slate-900 text-white pt-24 pb-40 px-4 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto flex flex-col items-center text-center relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter uppercase mb-4 leading-none flex items-center justify-center gap-3">
            {profile.full_name}
            {profile.is_verified && (
              <ShieldCheck size={36} className="text-blue-400 fill-blue-400/10" />
            )}
          </h1>

          <div className="flex flex-wrap justify-center gap-4 mb-8">
            <span className="px-5 py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-full shadow-lg shadow-blue-900/50">
            ...
            {profile.is_verified && (
              <span className="px-5 py-2 bg-white/10 border border-white/20 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-widest text-blue-300 flex items-center gap-2">
                <CheckCircle size={14} className="text-emerald-400"/> {"Verified Talent"}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-8 text-slate-400 font-bold uppercase tracking-[0.15em] text-[10px]">
            ...
            <span className="flex items-center gap-2"><Briefcase size={16} className="text-blue-500"/> {profile.career_status || "Professional"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-28 relative z-20 space-y-8">
        ...
        {/* VIDEO SECTION */}
        {videoId && (
          <div className="bg-white rounded-[3rem] p-4 shadow-2xl border border-slate-200 overflow-hidden">
            <div className="aspect-video w-full rounded-[2rem] overflow-hidden shadow-inner bg-black">
              <iframe 
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-200">
              ...
              <p className="text-lg text-slate-700 leading-relaxed font-medium italic italic-quote text-justify">
                {"\""}{profile.bio || `Saya adalah ${profile.full_name}, seorang profesional yang berdedikasi untuk berkarya di lingkungan kerja inklusif.`}{"\""}
              </p>
            </section>
                ...
                {profile.work_experiences?.length > 0 ? profile.work_experiences.map((work: any) => (
                  <div key={work.id} className="relative pl-10 border-l-2 border-slate-100">
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-2 mb-4">
                      ...
                      <span className="text-[10px] font-black uppercase px-3 py-1 bg-slate-100 text-slate-500 rounded-lg self-start">
                        {work.start_date} - {work.end_date || 'Sekarang'}
                      </span>
                    </div>
                    {work.description && <p className="text-sm text-slate-600 leading-relaxed text-justify bg-slate-50 p-5 rounded-2xl border border-slate-100">{work.description}</p>}
                    <div className="mt-3">
                      <span className="text-[9px] font-black uppercase text-slate-400 bg-white border border-slate-200 px-2 py-1 rounded">
                        {work.employment_type}
                      </span>
                    </div>
                  </div>
                )) : (
                  <p className="text-sm italic text-slate-400 uppercase font-bold tracking-widest text-center py-10">{"Data riwayat pekerjaan belum tersedia."}</p>
                )}
              </div>
            </section>
            ...
            <section className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-200">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {profile.certifications?.length > 0 ? profile.certifications.map((cert: any) => (
                  <div key={cert.id} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 hover:border-emerald-500 transition-all group">
                    ...
                    <div className="space-y-1">
                      <p className="text-[10px] font-bold text-slate-500 uppercase flex items-center gap-2">
                        <Building2 size={12}/> {cert.organizer_name}
                      </p>
                  ...
                  <div className="md:col-span-2 text-center py-6 text-sm italic text-slate-400 uppercase font-bold tracking-widest">{"Belum ada sertifikasi yang dicantumkan."}</div>
                )}
              </div>
            </section>
          </div>
          ...
          <div className="space-y-6">
            {/* AKOMODASI & AKSES (FOKUS RISET & AKSESIBILITAS) */}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    ) : <span className="text-xs font-bold italic text-slate-400">{"Tidak menggunakan alat bantu spesifik"}</span>}
                  </div>
                </div>
                <div className="pt-4 border-t border-slate-50">
                  ...
                  <p className="text-xs font-bold leading-relaxed text-slate-700 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    {profile.preferred_accommodation || "Terbuka untuk mendiskusikan penyesuaian lingkungan kerja agar optimal."}
                  </p>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-500 mt-1 uppercase">{profile.major}</p>
            ...
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200">
              ...
              <div className="flex flex-wrap gap-2">
                {profile.skills?.map((skill: string) => (
                  <span key={skill} className="px-3 py-1.5 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-tighter">
                    {skill}
                  </span>
                ))}
              </div>
              ...
              <div className="space-y-3">
                ...
                {profile.portfolio_url && (
                  <a href={profile.portfolio_url} target="_blank" className="flex items-center gap-3 p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all border border-white/5 group">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">{"Portfolio Website"}</span>
                  </a>
                )}
              </div>
              
              <div className="mt-8 pt-6 border-t border-white/10">
                <p className="text-[8px] font-black text-slate-500 uppercase mb-2 tracking-widest">{"Preferensi Komunikasi"}</p>
                <p className="text-xs font-bold italic text-blue-400 flex items-center gap-2">
                  <MessageSquare size={14}/> {profile.communication_preference || "Via disabilitas.com"}
                </p>
              </div>
            </section>
            ...
            <div className="text-center px-6">
              <p className="text-[7px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">{"DISABILITAS.COM PROFESSIONAL PROFILE"}</p>
              <p className="text-[6px] text-slate-400 italic leading-tight">
                {"Data ini ditampilkan dengan persetujuan penuh dari pemilik profil untuk tujuan profesional dan inklusivitas kerja."}
              </p>
            </div>
          </div>
        </div>
