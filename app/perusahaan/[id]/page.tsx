...
// 1. GENERATE METADATA DENGAN CANONICAL LINK
...
export default async function PublicCompanyProfile({ params }: { params: { id: string } }) {
  ...
  const inclusionRate = ((disabilityEmployees / totalEmployees) * 100).toFixed(1);
  ...
  // 4. PEMISAHAN DATA LOWONGAN (AKTIF & ARSIP)
        ...
        <div className="grid lg:grid-cols-3 gap-16">
            ...
            <section aria-labelledby="jobs-title" className="space-y-8">
              ...
              <div className="grid gap-6">
                {activeJobs.length > 0 ? activeJobs.map((job) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-8 shrink-0 w-full md:w-auto border-t md:border-t-0 md:border-l border-slate-50 pt-6 md:pt-0 md:pl-8">
                      <div className="text-left md:text-right flex-1 md:flex-none space-y-1">
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest italic text-center md:text-right">Estimasi Gaji</p>
                        <p className="text-xl font-black text-slate-900 flex items-center gap-1 justify-center md:justify-end">
                  ...
                  <div className="p-20 bg-slate-50 border-4 border-dashed border-slate-100 rounded-[4rem] text-center opacity-50 font-black uppercase italic text-slate-400">Belum ada lowongan riset aktif.</div>
                )}
              </div>
            </section>
          ...
          <aside className="space-y-12">
            <section className="bg-slate-900 rounded-[3.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-10 border-b border-white/10 pb-4 flex items-center gap-2">
                <BarChart3 size={16}/> Inclusion Analytic
              </h4>
              <div className="space-y-8 relative z-10">
                <div className="flex justify-between items-end border-l-2 border-blue-600 pl-6">
                  <div>
                    <p className="text-[34px] font-black italic leading-none">{disabilityEmployees}</p>
                    <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest mt-2">Staf Disabilitas</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[34px] font-black italic text-blue-400 leading-none">{inclusionRate}%</p>
                    <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest mt-2">Inclusion Rate</p>
                  </div>
                </div>
                <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                  <p className="text-[9px] font-medium text-slate-300 italic leading-relaxed text-center">
                    Data berdasarkan total {totalEmployees} karyawan yang terdaftar.
                  </p>
                </div>
              </div>
            </section>
            ...
            <section aria-labelledby="acc-title" className="bg-white border-2 border-slate-900 rounded-[3.5rem] p-10 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)] space-y-10">
              ...
              <ul className="space-y-4">
                ...
                )) : <p className="text-[10px] text-slate-300 uppercase italic font-black text-center">Data belum tersedia.</p>}
              </ul>
            </section>
