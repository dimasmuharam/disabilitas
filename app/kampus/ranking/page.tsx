...
export default async function CampusRankingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-emerald-100">
      ...
      <header className="relative border-b-8 border-slate-900 bg-emerald-600 px-4 py-20 text-white lg:py-32 overflow-hidden">
        <div className="mx-auto max-w-7xl relative z-10">
          <div className="flex flex-col items-start gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl space-y-6 text-left">
              ...
              <h1 className="text-6xl font-black uppercase italic tracking-tighter leading-none lg:text-9xl">
                National <br /> Leaderboard
              </h1>
              <p className="max-w-xl text-lg font-bold uppercase tracking-widest opacity-90 lg:text-2xl leading-tight">
                Memetakan Standar Pendidikan Tinggi Inklusif & Transisi Karir Disabilitas di Indonesia.
              </p>
            </div>
            <div className="hidden lg:block rotate-12 opacity-20">
               <BarChart2 size={300} />
            </div>
          </div>
        </div>
      ...
      <nav className="sticky top-0 z-20 border-b-4 border-slate-900 bg-white py-6">
        <div className="mx-auto max-w-7xl px-4 flex flex-col md:flex-row items-center justify-between gap-6">
           <div className="flex items-center gap-4 text-left">
              <div className="hidden sm:flex size-12 items-center justify-center rounded-xl bg-slate-100 text-slate-400">
                 <Search size={20} />
              </div>
              <div>
                 <h2 className="text-xl font-black uppercase italic tracking-tighter leading-none">Peringkat Terverifikasi</h2>
                 <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-slate-400 italic">Total: {campuses?.length} Institusi Unggulan</p>
              </div>
           </div>
           <div className="flex w-full md:w-auto items-center gap-3">
              <input 
                ...
                className="flex-1 md:w-80 rounded-2xl border-4 border-slate-900 bg-white px-6 py-3 text-sm font-bold outline-none shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] focus:ring-4 focus:ring-emerald-100 transition-all"
              />
           </div>
        </div>
      </nav>
        ...
        <div className="grid grid-cols-1 gap-8">
          {campuses?.map((campus, index) => {
            ...
            return (
              ...
              >
                {/* RANK & NAME */}
                <div className="flex items-center gap-8 md:w-2/5 shrink-0">
                  ...
                  <div className="text-left">
                    <h3 className="text-2xl font-black uppercase italic tracking-tighter leading-tight text-slate-900 group-hover:text-emerald-600 transition-colors">{campus.name}</h3>
                    <p className="mt-1 text-xs font-black uppercase tracking-widest text-slate-400 italic">{campus.location}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-1 flex-wrap justify-center gap-6 md:justify-center">
                   {[
                     ...
                     <div key={item.label} className="flex flex-col items-center gap-2">
                        <div className={`flex items-center gap-2 rounded-xl border-2 border-slate-900 px-4 py-2 bg-white text-[10px] font-black uppercase shadow-[3px_3px_0px_0px_rgba(15,23,42,1)]`}>
                          <item.icon size={12} className={`text-${item.color}-500`} />
                          {item.label}: {item.score}%
                        </div>
                        {item.label === "Digital" && isCriticalDigital && (
                          <span className="text-[8px] font-black text-orange-600 uppercase animate-pulse shrink-0">Butuh Audit!</span>
                        )}
                     </div>
                   ))}
                </div>
                ...
                {/* AGGREGATE SCORE */}
                <div className="flex items-center gap-8 md:w-1/4 md:justify-end shrink-0">
                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 leading-none">Final Index</p>
                ...
                {/* CHAMPION BADGE */}
                {index === 0 && (
                  <div className="absolute -top-6 left-12 rounded-2xl border-4 border-slate-900 bg-yellow-400 px-6 py-2 text-[11px] font-black uppercase italic shadow-lg animate-bounce">
                    🥇 No. 1 Nasional
                  </div>
                )}
              </Link>
      ...
      <section className="mx-auto max-w-7xl px-4 py-32">
        <div className="rounded-[4rem] border-8 border-slate-900 bg-slate-900 p-12 lg:p-24 text-white text-left relative overflow-hidden">
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8">
               ...
               <div className="space-y-6">
                 <div className="flex gap-4">
                    <div className="size-8 rounded-full bg-emerald-500 shrink-0 flex items-center justify-center font-black text-slate-900">1</div>
                 ...
                 <div className="flex gap-4">
                    <div className="size-8 rounded-full bg-blue-500 shrink-0 flex items-center justify-center font-black text-slate-900">2</div>
                    <p className="text-lg font-medium opacity-80 leading-relaxed italic">
                      <strong>Klaster Digital (40%):</strong> Audit standar WCAG pada website portal kampus dan platform LMS.
                    </p>
                 </div>
                 <div className="flex gap-4">
                    <div className="size-8 rounded-full bg-purple-500 shrink-0 flex items-center justify-center font-black text-slate-900">3</div>
                    <p className="text-lg font-medium opacity-80 leading-relaxed italic">
            ...
            <div className="rounded-[3rem] border-4 border-white/10 bg-white/5 p-10 space-y-6">
               ...
               <button className="w-full rounded-2xl bg-white py-5 text-[11px] font-black uppercase tracking-widest text-slate-900 hover:bg-emerald-400 transition-all">
