...
export default function TrainingContent({ training, partner, accommodations, skills }: any) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // JIKA ERROR (Termasuk RLS), tampilkan pesan di UI, bukan alert pop-up
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-8 pt-2">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left"><p className="text-[8px] font-black uppercase text-slate-400 mb-1">Penyelenggara</p><span className="text-sm font-black uppercase italic">{partner?.name || "Mitra"}</span></div>
              </div>
              <div className="flex items-center gap-3">
                <MapPin size={20} className="text-blue-600" />
                <div className="text-left"><p className="text-[8px] font-black uppercase text-slate-400 mb-1">Lokasi</p><span className="text-sm font-black uppercase italic">{training?.is_online ? "Akses Remote" : training?.location}</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-20 text-left grid grid-cols-1 gap-16 lg:grid-cols-3 font-sans">
        <div className="lg:col-span-2 space-y-20">
          <section aria-labelledby="desc-heading" className="space-y-6">
            <h2 id="desc-heading" className="flex items-center gap-4 text-2xl font-black uppercase italic tracking-tighter text-slate-900"><FileText className="text-blue-600" size={32} /> Tentang Program</h2>
          ...
          {skills?.length > 0 && (
            <section aria-labelledby="skill-heading" className="space-y-8">
              <h2 id="skill-heading" className="flex items-center gap-4 text-2xl font-black uppercase italic tracking-tighter text-slate-900"><Target className="text-blue-600" size={32} /> Skill Yang Didapat</h2>
          ...
          <section aria-labelledby="syllabus-heading" className="space-y-8">
            <h2 id="syllabus-heading" className="flex items-center gap-4 text-2xl font-black uppercase italic tracking-tighter text-slate-900"><BookOpen className="text-blue-600" size={32} /> Kurikulum Pembelajaran</h2>
            <div className="grid gap-4">
              {training?.syllabus?.split('\n').filter((s: string) => s.trim() !== "").map((item: string, idx: number) => (
                <div key={idx} className="flex items-center gap-6 rounded-3xl border-2 border-slate-50 bg-white p-6 hover:border-blue-600 transition-all group">
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-sm font-black text-white group-hover:bg-blue-600 transition-colors">{idx + 1}</span>
        ...
        <aside className="space-y-10">
          {/* PANEL PENDAFTARAN - Heading Aksesibel */}
          <section aria-labelledby="reg-heading" className="rounded-[3.5rem] bg-slate-900 p-10 text-white shadow-2xl border-t-[12px] border-blue-600">
            <h2 id="reg-heading" className="mb-8 flex items-center gap-2 border-b border-white/10 pb-4 text-[10px] font-black uppercase tracking-widest italic text-blue-400">
              <Clock size={16} /> Panel Pendaftaran
            </h2>
            
            <div className="space-y-6">
              {/* Pesan Error Inline (Aksesibel) */}
              {errorMessage && (
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-[8px] font-black uppercase opacity-60 mb-1 italic">Masa Pendaftaran</p>
                  <p className="text-[11px] font-bold text-emerald-400">BUKA: {formatFullDate(training?.registration_start)}</p>
                  <p className="text-[11px] font-bold text-red-400 uppercase">TUTUP: {formatFullDate(training?.registration_deadline)}</p>
                </div>
                ...
                {!isExpired ? (
                  <button 
                    ...
                    aria-label={isSubmitting ? "Sedang memproses..." : "Daftar pelatihan ini"}
                  ...
                  <div className="rounded-2xl bg-slate-800 p-6 text-center border border-white/10">
                    <p className="text-[10px] font-black uppercase italic text-slate-500">Masa Pendaftaran Berakhir</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <section aria-labelledby="schedule-heading" className="rounded-[3.5rem] bg-blue-600 p-10 text-white shadow-xl space-y-8">
            <h3 id="schedule-heading" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest italic border-b border-blue-400 pb-4"><Timer size={16} /> Jadwal Pelaksanaan</h3>
            <div className="space-y-4">
              <p className="text-base font-black italic uppercase leading-tight">{formatFullDate(training?.start_date)} - {formatFullDate(training?.end_date)}</p>
              {training?.start_time && (
          ...
          <section aria-labelledby="inclusion-heading" className="rounded-[3.5rem] border-2 border-slate-900 bg-white p-10 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)] space-y-8">
            <h3 id="inclusion-heading" className="flex items-center gap-3 text-lg font-black uppercase tracking-tighter text-slate-900"><Accessibility className="text-blue-600" size={24} /> Fasilitas Inklusi</h3>
            <ul className="space-y-4" aria-label="Daftar akomodasi yang disediakan">
              {accommodations?.map((item: string, idx: number) => (
                <li key={idx} className="flex items-start gap-4 rounded-2xl border-2 border-emerald-50 bg-emerald-50/20 p-4 shadow-sm">
                  ...
                  <span className="text-[11px] font-black uppercase text-emerald-950 italic">{item}</span>
                </li>
              ))}
            </ul>
          </section>
