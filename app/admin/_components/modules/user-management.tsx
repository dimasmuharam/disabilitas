          ...
          {paginatedData.map((user: any) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2 pt-4 border-t-2 border-slate-50 lg:pt-0 lg:border-t-0">
                ...
                <ActionButton icon={<Key size={12}/>} label="Reset Pass" onClick={() => onAction("RESET_PASSWORD", user.email)} color="hover:bg-amber-500" />
...
function ActionButton({ icon, label, onClick, color }: any) {
  return (
    <button 
      onClick={onClick} 
      className={cn(
        "flex items-center gap-2 rounded-xl bg-slate-100 px-3 py-2 text-[8px] font-black uppercase text-slate-600 transition-all hover:text-white",
        color
      )}
    >
