...
import { AlertTriangle, ArrowRight, Database, History } from "lucide-react"

export default function AuditHub({ logs = [], onMerge, canAction = true }: any) {
  return (
              ...
              {logs.length > 0 ? logs.map((log: any) => (
                <tr key={log.id} className="transition-all hover:bg-slate-50">
                  ...
                  <td className="px-10 py-6 text-right">
                    {/* canAction default true karena lolos Cloudflare adalah Admin */}
                    {canAction ? (
                      <button 
                        onClick={() => onMerge(log)}
