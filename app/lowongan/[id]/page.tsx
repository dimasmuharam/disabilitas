...
export default function JobDetailPage({ params }: { params: { id: string } }) {
  ...
  const MSG_CONFIRM = "Dengan melamar, Anda setuju membagikan profil profesional Anda untuk kepentingan rekrutmen dan data riset inklusivitas.";
  const MSG_SUCCESS = "Lamaran berhasil terkirim.";
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        let query = supabase
          .from('jobs')
          .select(`*, companies (*)`) // Pastikan relasi companies terambil
  ...
  async function handleApply() {
    ...
    if (!error) { setHasApplied(true); alert(MSG_SUCCESS); }
    setApplying(false);
  }

  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-white font-black uppercase italic text-slate-400 animate-pulse">
      {TXT_LOADING}
    </div>
  );
  ...
  if (!job) return (
    <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
      ...
      <Link href="/lowongan" className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black uppercase text-[10px]">{TXT_BACK}</Link>
    </div>
  );
    ...
    <main className="min-h-screen bg-[#FDFDFD] pb-24 pt-10 font-sans text-left">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <Link href="/lowongan" className="inline-flex items-center text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 mb-10 transition-all group">
          <ArrowLeft className="mr-2 h-4 w-4 group-hover:-translate-x-1" /> {TXT_BACK}
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          <div className="lg:col-span-2 space-y-8">
            ...
            <article className="bg-white rounded-[3rem] p-8 md:p-12 border-2 border-slate-100 shadow-sm space-y-6">
              <div className="flex flex-wrap gap-3">
                <span className="bg-blue-50 text-blue-700 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase border border-blue-100 italic">{job.job_type}</span>
                <span className="bg-emerald-50 text-emerald-700 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase border border-emerald-100 italic">{job.work_mode}</span>
              </div>
            ...
            <section className="bg-white rounded-[3rem] p-8 md:p-12 border-2 border-slate-100 shadow-sm grid md:grid-cols-2 gap-10">
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(job.required_education_major) && job.required_education_major.map((m: string) => (
              ...
              <div className="space-y-4 border-t md:border-t-0 md:border-l-2 border-slate-50 pt-8 md:pt-0 md:pl-10">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(job.required_skills) && job.required_skills.map((s: string) => (
                    <span key={s} className="px-3 py-1 bg-slate-900 text-white text-[9px] font-black uppercase rounded-lg italic">{s}</span>
                  ))}
                </div>
              </div>
            </section>
            ...
            <section className="bg-white rounded-[3rem] p-8 md:p-12 border-2 border-slate-100 shadow-sm space-y-10">
              <div className="space-y-4">
                ...
                <div className="text-slate-700 whitespace-pre-line leading-relaxed font-medium text-lg">{job.description}</div>
              </div>
              {job.requirements && (
          ...
          <aside className="space-y-8 sticky top-10">
            <div className="bg-slate-900 text-white rounded-[3.5rem] p-10 shadow-2xl">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-8 border-b border-white/10 pb-4">Panel Lamaran</h3>
              {hasApplied ? (
                <div className="w-full py-6 bg-emerald-500/20 text-emerald-400 rounded-3xl font-black text-center text-xs flex flex-col items-center gap-2 uppercase tracking-widest">
                  <CheckCircle size={24}/> Sudah Melamar
                </div>
              ) : (
                <button onClick={handleApply} disabled={applying || !job.is_active} className="w-full h-16 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-800 text-white rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95 text-xs">
            ...
            <section className="bg-white rounded-[3rem] p-10 border-2 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] space-y-6 text-left">
              ...
              <div className="space-y-3">
                {Array.isArray(job.companies?.master_accommodations_provided) ? job.companies.master_accommodations_provided.map((acc: string) => (
                  <div key={acc} className="flex items-center gap-3 text-[10px] font-black uppercase text-slate-700">
                    <CheckCircle size={14} className="text-emerald-500 shrink-0" /> {acc}
                  </div>
                )) : <p className="text-[10px] italic text-slate-400">Belum ada data akomodasi.</p>}
              </div>
            </section>
          </aside>
        </div>
