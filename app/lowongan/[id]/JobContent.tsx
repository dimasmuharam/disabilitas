...
export default function JobContent({ job, skills, majors, accommodations }: any) {
  ...
  const handleApply = async () => {
    ...
    setApplying(true);
    try {
      // Sesuai Skema: Pastikan kolom sesuai dengan tabel 'applications'
      ...
      // Aksesibilitas: Pindahkan fokus ke pesan sukses agar dibaca Screen Reader
  ...
  // --- TAMPILAN KONFIRMASI (BUKAN POP-UP, TAPI FULL PAGE STATE) ---
  if (isSuccess) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#FDFDFD] px-6 py-20 font-sans text-slate-900">
        <div 
          ref={successRef} 
          tabIndex={-1} 
          role="alert"
        ...
        >
          ...
          <h1 className="mb-4 text-4xl font-black uppercase italic tracking-tighter md:text-5xl">Lamaran Berhasil!</h1>
          <p className="mb-10 text-lg font-bold italic text-slate-500 uppercase tracking-tight leading-tight">
            Data profil Anda telah terkirim ke sistem rekrutmen <span className="text-blue-600 underline">{job.companies?.name}</span>.
          </p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            >
              <Search size={18} /> Cari Lowongan Lagi
            </Link>
          </div>
          
          <p className="mt-8 text-[10px] font-black uppercase italic text-slate-300 tracking-widest">
            Terima kasih telah berkontribusi dalam pengembangan talenta inklusif
          </p>
        </div>
      </main>
    );
          ...
          <div className="space-y-8 lg:col-span-2">
            <article className="rounded-[3rem] border-2 border-slate-100 bg-white p-8 shadow-sm md:p-12 space-y-6">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-xl font-bold text-blue-600 underline underline-offset-8 decoration-2">{job.companies?.name}</span>
              </div>
          ...
          <aside className="sticky top-10 space-y-8">
            <div className="rounded-[3.5rem] bg-slate-900 p-10 text-white shadow-2xl space-y-8">
              ...
              {hasApplied ? (
                <div className="flex w-full flex-col items-center gap-3 rounded-2xl bg-emerald-500/10 py-8 text-xs font-black italic text-emerald-400 border border-emerald-500/20">
                  <CheckCircle size={32} aria-hidden="true"/> LAMARAN TERKIRIM
                </div>
              ) : (
                <button 
                  onClick={handleApply} 
              ...
              <p className="text-center text-[8px] font-bold uppercase text-slate-500 leading-relaxed italic">
                Data profil Anda akan otomatis dibagikan kepada rekruter.
              </p>
            </div>
            ...
            <section className="rounded-[3rem] border-2 border-slate-900 bg-white p-10 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] space-y-6">
              ...
              <div className="space-y-3">
                ...
                )) : <p className="text-[10px] italic text-slate-300 uppercase font-bold">Belum ada akomodasi terdaftar.</p>}
              </div>
            </section>
          </aside>
        </div>
