...
import { 
  ...
  Search, Clock, Building2, ArrowRight, Share2, Send, QrCode
} from "lucide-react"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl p-8 border-4 border-blue-600 shadow-2xl">
            ...
            <p className="text-sm text-slate-600 mb-8 leading-relaxed">
              Saya setuju data saya digunakan untuk rekrutmen inklusif dan diolah sebagai data anonim riset disabilitas.com demi kemajuan kebijakan inklusi nasional.
            </p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.4rem] p-8 flex flex-col md:flex-row gap-8 items-center relative z-10">
                ...
                <div className="w-full md:w-72 aspect-[3/4] bg-slate-950 rounded-3xl p-6 text-white flex flex-col justify-between shadow-2xl relative overflow-hidden border border-white/10">
                    <div className="flex justify-between items-start">
                        <div className="bg-blue-600 p-2 rounded-lg font-black text-[10px] italic">DISABILITAS.COM</div>
                    ...
                    <div>
                        ...
                        <div className="space-y-1">
                            <p className="text-[8px] text-slate-500 uppercase font-black">Top Expertise</p>
                            <p className="text-[10px] font-bold truncate">{skills.split(',')[0] || "General Talent"}</p>
                        </div>
                    </div>
                    <div className="flex justify-between items-end border-t border-white/10 pt-4">
                        <div className="bg-white p-1 rounded-lg">
                           <QRCodeSVG value={publicProfileUrl} size={64} aria-label="QR Code Profil Publik" />
                        </div>
                        <p className="text-[8px] font-black text-slate-500 text-right uppercase">Verified<br/>Talent ID</p>
                    </div>
                </div>
                ...
                <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-slate-500 font-medium mb-6 leading-relaxed max-w-lg">
                        Profil kepakaran Anda kini telah terstandarisasi secara profesional. Bagikan kartu ID digital ini untuk menunjukkan kompetensi Anda sebagai talenta inklusif yang bonafit.
                    </p>
                    <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                        ...
                        <a href={publicProfileUrl} target="_blank" className="bg-slate-100 text-slate-800 px-8 py-4 rounded-2xl font-black uppercase tracking-widest flex items-center gap-3 hover:bg-slate-200 transition-all border border-slate-200">
                            <ExternalLink size={20}/> Lihat Profil Publik
                        </a>
                    </div>
                </div>
            </div>
      ...
      {isEditing ? (
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm animate-in slide-in-from-top-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    ...
                    <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400">Kota Domisili (Hybrid)</label><input list="city-list" value={city} onChange={e => setCity(e.target.value)} className="input-std" /><datalist id="city-list">{INDONESIA_CITIES.map(c => <option key={c} value={c} />)}</datalist></div>
                </div>
                <div className="space-y-4">
                    <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400">Ragam Disabilitas (Hybrid)</label><input list="dis-list" value={disabilityType} onChange={e => setDisabilityType(e.target.value)} className="input-std" /><datalist id="dis-list">{DISABILITY_TYPES.map(t => <option key={t} value={t} />)}</datalist></div>
                    <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400">Pendidikan</label><select value={lastEducation} onChange={e => setLastEducation(e.target.value)} className="input-std">{EDUCATION_LEVELS.map(l => <option key={l} value={l}>{l}</option>)}</select></div>
                </div>
            </div>
            <button onClick={handleSaveProfile} disabled={saving} className="w-full mt-10 h-16 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-xl">
                {saving ? "Sinkronisasi..." : "Simpan Pembaruan Profil"}
            </button>
        </div>
      ) : (
        <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                <section>
                    <h2 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-4 flex items-center gap-2"><Clock size={16}/> Pelacakan Lamaran</h2>
                    <div className="grid gap-4">
                        ...
                        )) : <div className="p-10 border-2 border-dashed border-slate-200 rounded-3xl text-center text-slate-400 font-medium italic">Belum ada lamaran.</div>}
                    </div>
                </section>

                <section>
                    <h2 className="text-sm font-black uppercase tracking-widest text-blue-600 mb-4 flex items-center gap-2"><Search size={16}/> Rekomendasi Lowongan</h2>
                        ...
                        {recommendedJobs.map(job => (
                            <div key={job.id} className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100 group transition-all">
                                ...
                                <button 
                                    ...
                                    className="w-full py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-blue-700 disabled:bg-slate-300 transition-all"
                                >
            ...
            <div className="space-y-8">
                <section className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                    <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 border-b pb-2">Kepakaran</h2>
                    <div className="space-y-4">
                        ...
                        <div><p className="text-[9px] font-black text-blue-600 uppercase mb-1">Alat Bantu</p><p className="text-xs font-bold">{assistiveTools || "Kosong"}</p></div>
                    </div>
                </section>
