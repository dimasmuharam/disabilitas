...
export default function TalentDashboard({ user, profile: initialProfile, autoOpenProfile }: TalentDashboardProps) {
  ...
  useEffect(() => {
    if (activeTab !== "overview") {
      setTimeout(() => moduleHeaderRef.current?.focus(), 150);
    }
  }, [activeTab]);
  ...
  async function fetchLatestData() {
    try {
      ...
      if (prof) {
        ...
        // PERBAIKAN SMART-MATCH: Dibuat lebih fleksibel (OR condition antara Lokasi, Pendidikan, atau Skill)
      ...
      // PERBAIKAN TRACKING: Memastikan companies/partners terpanggil untuk Link Profil Publik
...
const renderContent = () => {
    switch (activeTab) {
      case "identity": return <IdentityLegal user={user} profile={profile} onSuccess={handleBackToOverview} />;
      case "tech": return <TechAccess user={user} profile={profile} onSuccess={handleBackToOverview} />;
      case "career": return <CareerExperience user={user} profile={profile} onSuccess={handleBackToOverview} />;
      case "academic": return <AcademicBarriers user={user} profile={profile} onSuccess={handleBackToOverview} />;
      case "skills": return <SkillsCertifications user={user} profile={profile} onSuccess={handleBackToOverview} />;
      case "settings": return <AccountSettings user={user} onSuccess={handleBackToOverview} />;
      default: return (
        <div className="space-y-10 duration-500 animate-in fade-in">
          {/* FITUR KELENGKAPAN PROFIL */}
          ...
          <div className="grid gap-8 text-left md:grid-cols-2">
            {/* JOBS TRACKING - PERBAIKAN LINK & STATUS */}
            <div className="space-y-4">
              ...
              <div className="min-h-[300px] overflow-hidden rounded-[2.5rem] border-2 border-slate-100 bg-white shadow-inner">
                      ...
                      <div key={app.id} className="p-6 transition-colors hover:bg-slate-50">
                        <div className="flex items-start justify-between gap-4 text-left">
                          <div className="flex-1 space-y-1">
                            ...
                            {/* PERUSAHAAN -> LINK PUBLIC PROFILE PERUSAHAAN */}
                        ...
                        {app.hrd_notes && (
                          <div className="mt-4 flex items-start gap-3 rounded-2xl border border-amber-100 bg-amber-50/50 p-4">
                            ...
                            <div className="space-y-1 text-left">
                              <p className="text-[9px] font-black uppercase text-amber-700 italic">Pesan HRD / Jadwal:</p>
                ...
                ) : <div className="flex h-[300px] flex-col items-center justify-center p-10 opacity-30 font-bold uppercase italic"><Briefcase size={32} className="mb-2"/> Belum ada lamaran.</div>}
              </div>
            </div>

            {/* TRAININGS TRACKING - PERBAIKAN LINK & STATUS */}
            <div className="space-y-4">
              <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-900">
              ...
              <div className="min-h-[300px] overflow-hidden rounded-[2.5rem] border-2 border-slate-100 bg-white shadow-inner">
                  ...
                  <div className="divide-y divide-slate-50">
                    {appliedTrainings.map((reg) => (
                      <div key={reg.id} className="p-6 transition-colors hover:bg-slate-50 text-left">
                        <div className="flex items-start justify-between gap-4 text-left">
                          <div className="flex-1 space-y-1">
                            {/* PELATIHAN -> LINK DETAIL PELATIHAN */}
                ...
                ) : <div className="flex h-[300px] flex-col items-center justify-center p-10 opacity-30 font-bold uppercase italic"><BookOpen size={32} className="mb-2"/> Belum ada pelatihan.</div>}
              </div>
            </div>
          </div>

          {/* SMART MATCH SECTION - PERBAIKAN LOGIKA REKOMENDASI */}
          <section className="space-y-6 border-t-2 border-slate-50 pt-10 text-left">
            ...
            <div className="grid gap-8 md:grid-cols-2">
              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase tracking-tighter text-slate-400 italic">Peluang Karir Lokal/Sesuai Pendidikan</p>
                ...
                )) : <p className="text-[10px] italic text-slate-300">Belum ada peluang yang cocok.</p>}
              </div>
              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase tracking-tighter text-slate-400 italic">Pelatihan Relevan Sesuai Disabilitas/Lokasi</p>
  ...
  // ... (Sisa file return JSX Header, Sidebar Nav, dan Modal Rating tetap sama seperti sebelumnya) ...
  return (
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 pt-8 text-left text-slate-900">
           ...
           <div className="flex flex-col items-start gap-8 md:flex-row md:items-center">
            ...
            <div className="flex-1 space-y-2">
              ...
              <p className="max-w-xl text-sm font-bold italic leading-relaxed text-slate-500">
                {profile?.summary || "Silakan lengkapi profil singkat Anda di menu Identitas."}
              </p>
        ...
        <div className="grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2">
            {activeTab !== "overview" && (
              <h2 ref={moduleHeaderRef} tabIndex={-1} className="mb-6 flex items-center gap-2 text-[10px] font-black uppercase text-slate-900 outline-none">
            ...
            <div className="rounded-[3rem] border-t-[12px] border-blue-600 bg-slate-900 p-8 text-white shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => generateProfessionalCV(profile, workExps, certifications)} disabled={isProcessing} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-white p-4 text-[10px] font-black uppercase text-slate-900 shadow-lg transition-all hover:bg-blue-50">
                  <FileDown size={18} /> {isProcessing ? "Menyusun PDF..." : "Cetak CV"}
                </button>
