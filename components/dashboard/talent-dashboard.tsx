...
import { 
  ...
  LayoutDashboard, Share2, ExternalLink, ShieldCheck, Clock, Timer, MessageCircle
} from "lucide-react";
...
export default function TalentDashboard({ user, profile: initialProfile, autoOpenProfile }: TalentDashboardProps) {
  ...
  async function fetchLatestData() {
    try {
      ...
      if (prof) {
        ...
        // --- SMART MATCH FLEKSIBEL v2.1 (OR Logic) ---
        const { data: recJobs } = await supabase
          .from("jobs")
          .select("*, companies(name)")
          .eq("is_active", true)
          .or(`location.ilike.%${prof.city}%,required_education_level.eq.${prof.education_level},required_education_major.ilike.%${prof.major}%`)
          .limit(4);
  ...
  function calculateProgress(p: any) {
    const fields = [
      { v: p?.full_name, l: "Nama", m: "Identitas" },
      { v: p?.city, l: "Domisili", m: "Identitas" },
      { v: p?.disability_type, l: "Ragam", m: "Identitas" },
      { v: p?.resume_url, l: "CV", m: "Karir" },
      { v: p?.skills?.length > 0, l: "Skill", m: "Skill" }
    ];
    const missing = fields.filter(f => !f.v).map(f => `${f.m}: ${f.l}`);
    setMissingFields(missing);
    setProgress(Math.round(((fields.length - missing.length) / fields.length) * 100));
  }
    ...
    switch (activeTab) {
      ...
      default: return (
        <div className="space-y-10 animate-in fade-in duration-500">
          ...
          <section className="relative overflow-hidden rounded-[3rem] border-2 border-slate-900 bg-white p-10 shadow-sm">
            <div className="flex flex-col items-start gap-8 md:flex-row">
              <div className="flex size-20 shrink-0 items-center justify-center rounded-3xl bg-blue-600 text-3xl font-black italic text-white shadow-xl">
                    ...
                    {appliedJobs.map((app) => (
                      <div key={app.id} className="flex items-center justify-between p-5 hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex items-center gap-2">
                           {(app.status === 'accepted' || app.status === 'rejected') && !ratedJobs.includes(app.id) && (
                             <button onClick={() => setSelectedJobRating(app)} className="rounded-full bg-amber-100 px-3 py-1 text-[8px] font-black uppercase text-amber-700 hover:bg-amber-600 hover:text-white transition-all">Audit Inklusi</button>
                           )}
          ...
          {/* SMART MATCH */}
          <section className="space-y-6 border-t-2 border-slate-50 pt-10">
            <h3 className="flex items-center gap-2 text-xs font-black uppercase italic tracking-widest text-slate-900">
              <Search size={16} className="text-blue-600" /> Rekomendasi Pintar (Smart Match)
            </h3>
            <div className="grid gap-8 md:grid-cols-2 text-left">
              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase text-slate-400">Lowongan Untuk Anda</p>
                {recommendedJobs.map((j) => (
                  <Link href={`/lowongan/${j.slug || j.id}`} key={j.id} className="group block rounded-[2rem] border-2 border-slate-100 bg-white p-5 hover:border-blue-600 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <p className="text-xs font-black uppercase text-slate-900 group-hover:text-blue-600">{j.title}</p>
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase text-slate-400">Pelatihan Relevan</p>
                {recommendedTrainings.map((t) => (
                  <Link href={`/pelatihan/${t.slug || t.id}`} key={t.id} className="group block rounded-[2rem] border-2 border-slate-100 bg-white p-5 hover:border-emerald-600 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <p className="text-xs font-black uppercase text-slate-900 group-hover:text-emerald-600">{t.title}</p>
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] pb-24 font-sans selection:bg-blue-100">
        ...
        <section className="rounded-[3.5rem] border-2 border-slate-900 bg-white p-10 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)]">
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
            ...
            <div className="flex w-full items-center gap-4 md:w-auto">
              ...
              <span className="text-3xl font-black italic text-slate-900">{`${progress}%`}</span>
            </div>
          </div>
          ...
          {activeTab === "overview" && (
            <nav className="mt-10 grid grid-cols-2 gap-4 border-t border-slate-100 pt-10 md:grid-cols-6">
              {[
                ...
                <button key={m.id} onClick={() => setActiveTab(m.id)} aria-label={`Buka modul ${m.label}`} className="group rounded-3xl border-2 border-transparent bg-slate-50 p-5 text-center transition-all hover:border-blue-600 hover:bg-white">
          ...
          {activeTab !== "overview" && (
            <button onClick={() => setActiveTab("overview")} aria-label="Kembali ke overview dashboard" className="mt-8 flex items-center gap-2 text-xs font-black uppercase text-blue-600 hover:gap-3 transition-all">
              <ChevronLeft size={16}/> Kembali ke Overview Utama
            </button>
          )}
        </section>
        ...
        <div className="grid gap-10 lg:grid-cols-3">
          ...
          <aside className="space-y-8">
            ...
            <div className="rounded-[3rem] bg-slate-900 p-8 text-white shadow-2xl border-t-[12px] border-blue-600">
              <h3 className="mb-6 text-[10px] font-black uppercase tracking-widest text-blue-400">Professional Actions</h3>
              <div className="space-y-3">
                ...
                <button onClick={() => generateProfessionalCV(profile, workExps, certifications)} disabled={isProcessing} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-white p-4 text-[10px] font-black uppercase text-slate-900 hover:bg-blue-50 transition-all">
                  <FileDown size={18} /> {isProcessing ? "Menyusun PDF..." : "Cetak CV Profesional"}
                </button>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => handleNativeShare(user.id, profile)} aria-label="Share kartu profil" className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 p-4 text-[8px] font-black uppercase hover:bg-emerald-700 transition-all">
                    <Share2 size={16} /> Native Share
                  </button>
                  <button onClick={() => handleWhatsAppShare(user.id, profile)} aria-label="Share ke WhatsApp" className="flex items-center justify-center gap-2 rounded-2xl bg-[#25D366] p-4 text-[8px] font-black uppercase hover:opacity-90 transition-all">
            ...
            {/* SUMMARY STATS */}
            <div className="rounded-[3rem] border-2 border-slate-200 bg-white p-8 text-left shadow-sm">
              <h3 className="mb-6 flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 italic tracking-widest">
                <LayoutDashboard size={14}/> Ringkasan Aktivitas
              </h3>
      ...
      {/* HIDDEN INCLUSION CARD FOR CAPTURE */}
      <div className="pointer-events-none absolute -z-50 overflow-hidden opacity-0" aria-hidden="true">
         <div id="inclusion-card-capture" className="flex h-[400px] w-[700px] flex-col justify-between border-[16px] border-slate-900 bg-white p-12 font-sans text-slate-900">
            <div className="flex items-center justify-between border-b-4 border-blue-600 pb-6">
               <h2 className="text-3xl font-black uppercase italic tracking-tighter text-blue-600">disabilitas.com</h2>
               <span className="rounded-full bg-blue-600 px-6 py-2 text-xs font-black uppercase text-white shadow-lg">Verified Talent</span>
            </div>
            <div className="flex flex-1 items-center gap-10 py-8">
            ...
            <div className="flex items-end justify-between border-t-2 border-slate-100 pt-6 text-left">
               <div className="space-y-1">
                  <p className="text-[11px] font-black uppercase">Inclusion Identity Card 2026</p>
