...
export default function TalentDashboard({ user, profile: initialProfile, autoOpenProfile }: TalentDashboardProps) {
  ...
  // --- LOGIKA PROGRESS PROFILE & DETEKSI BAGIAN KOSONG ---
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "identity": return <IdentityLegal user={user} profile={profile} onSuccess={handleBack} />;
      case "tech": return <TechAccess user={user} profile={profile} onSuccess={handleBack} />;
      case "career": return <CareerExperience user={user} profile={profile} onSuccess={handleBack} />;
      case "academic": return <AcademicBarriers user={user} profile={profile} onSuccess={handleBack} />;
      case "skills": return <SkillsCertifications user={user} profile={profile} onSuccess={handleBack} />;
      case "settings": return <AccountSettings user={user} onSuccess={handleBack} />;
      default: return (
        <div className="space-y-10 duration-500 animate-in fade-in">
          
          {/* FITUR BARU: CEK KELENGKAPAN PROFIL (List Spesifik Bagian Kosong) */}
          {completionData.percent < 100 && (
            <div className="rounded-[2.5rem] border-2 border-amber-200 bg-amber-50 p-8 text-left" role="alert">
              ...
              <p className="mb-4 text-xs font-medium text-amber-600 italic">Screen reader: Mohon lengkapi bagian-bagian berikut agar profil Anda dapat dilirik rekruter.</p>
          ...
          <div className="grid gap-8 text-left md:grid-cols-2">
            {/* JOBS TRACKING (Link Terpisah: Posisi vs Perusahaan) */}
            <div className="space-y-4">
              ...
              <div className="min-h-[300px] overflow-hidden rounded-[2.5rem] border-2 border-slate-100 bg-white">
                ...
                ) : <div className="flex h-[300px] items-center justify-center p-10 opacity-30 font-bold uppercase italic text-center text-[10px]">Belum ada lamaran.</div>}
              </div>
            </div>

            {/* TRAININGS TRACKING (Link Terpisah: Pelatihan vs Partner) */}
            <div className="space-y-4">
              <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-900">
              ...
              <div className="min-h-[300px] overflow-hidden rounded-[2.5rem] border-2 border-slate-100 bg-white">
                ...
                ) : <div className="flex h-[300px] items-center justify-center p-10 opacity-30 font-bold uppercase italic text-center text-[10px]">Belum ada pelatihan.</div>}
              </div>
            </div>
          </div>

          {/* SMART MATCH SECTION (Fitur Asli Tidak Berkurang) */}
          <section className="space-y-6 border-t-2 border-slate-50 pt-10 text-left">
            ...
            <div className="grid gap-8 md:grid-cols-2">
              <div className="space-y-4">
                ...
                )) : <p className="text-[10px] italic text-slate-400">Belum ada lowongan sesuai domisili.</p>}
              </div>
              <div className="space-y-4">
                ...
                )) : <p className="text-[10px] italic text-slate-400">Belum ada pelatihan sesuai ragam disabilitas.</p>}
              </div>
            </div>
          </section>
        </div>
  ...
  if (loading) return <div role="status" className="animate-pulse p-20 text-center font-black uppercase italic text-slate-400">Sinkronisasi Portal Karir...</div>;

  return (
    <div className="min-h-screen bg-[#FDFDFD] pb-24">
      <div className="mx-auto max-w-6xl space-y-8 px-4 pt-8 text-left">
        
        {/* HEADER PROFILE (DATA NAMA & BIO LENGKAP) */}
        <header className="rounded-[3.5rem] border-2 border-slate-900 bg-white p-10 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)]">
           <div className="flex flex-col items-start gap-8 md:flex-row md:items-center">
            ...
            <div className="flex-1 text-left">
              ...
              <p className="mt-2 max-w-xl text-sm font-bold italic leading-relaxed text-slate-500">
                {profile?.summary || profile?.bio || "Lengkapi profil singkat Anda di menu Identitas."}
              </p>
              <div className="flex flex-wrap gap-4 pt-4">
                <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-slate-400">
                  <MapPin size={12} className="text-red-500" /> {profile?.city || "Lokasi Kosong"}
                </span>
                <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-slate-400">
                  <Briefcase size={12} className="text-blue-500" /> {profile?.career_status || "Status Kosong"}
                </span>
              </div>
            </div>
            <div className="flex flex-col items-end gap-3">
               <div className="flex items-center gap-3">
                 <span className="text-3xl font-black italic">{completionData.percent}%</span>
                 <div className="h-4 w-32 overflow-hidden rounded-full border border-slate-200 bg-slate-100" role="progressbar" aria-valuenow={completionData.percent} aria-valuemin={0} aria-valuemax={100}>
                    <div className="h-full bg-blue-600 transition-all duration-1000" style={{ width: `${completionData.percent}%` }}></div>
                 </div>
               </div>
               <p className="text-[8px] font-black uppercase text-slate-400 italic">Kualitas Kelengkapan Profil</p>
            </div>
           </div>
        </header>

        {/* NAVIGATION TABS */}
        <nav className="grid grid-cols-3 gap-4 md:grid-cols-6" aria-label="Menu Utama Dashboard">
          {[
            ...
            <button 
              ...
              className={`flex flex-col items-center justify-center rounded-3xl border-2 p-5 transition-all ${
        ...
        <div className="grid gap-10 lg:grid-cols-3">
          ...
          <aside className="space-y-8">
            {/* PROFESSIONAL HUB */}
            <div className="rounded-[3.5rem] border-t-[12px] border-blue-600 bg-slate-900 p-8 text-white shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => generateProfessionalCV(profile, workExps, certifications)} disabled={isProcessing} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-white p-4 text-[10px] font-black uppercase text-slate-900 shadow-lg transition-all hover:bg-blue-50">
                  <FileDown size={18} /> {isProcessing ? "Menyusun..." : "Download CV PDF"}
                </button>
            ...
            {/* RINGKASAN AKTIVITAS */}
            <div className="rounded-[3.5rem] border-2 border-slate-100 bg-white p-8 shadow-sm">
