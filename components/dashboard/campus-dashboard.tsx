...
export default function CampusDashboard({ user }: { user: any }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-10 px-4 py-10 text-slate-900 selection:bg-emerald-100 font-sans">
      ...
      {/* HEADER SECTION */}
      <header className="flex flex-col items-start justify-between gap-6 border-b-4 border-slate-900 pb-10 md:flex-row md:items-end text-left">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button 
            ...
            className="rounded-2xl border-2 border-slate-100 bg-white px-6 hover:border-slate-900 transition-all shadow-sm"
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-5">
              ...
              <section className="lg:col-span-2 rounded-[3rem] border-2 border-slate-100 bg-white p-8 shadow-sm">
                <header className="mb-6 text-left">
                  <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 flex items-center gap-2 leading-none">
            ...
            <section aria-label="Metrik Kunci Akademik" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 text-left">
              {[
                ...
                <div key={item.label} className="rounded-[2.5rem] border-2 border-slate-100 bg-white p-8 shadow-sm text-left">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{item.label}</p>
                  <div className="mt-2 flex items-end justify-between">
                    <p className="text-5xl font-black tracking-tighter">{item.score || 0}%</p>
                    <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-lg bg-slate-50 text-slate-600`}>{item.sub}</span>
                  </div>
                </div>
              ))}
            </section>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 text-left">
               <div className="lg:col-span-2 rounded-[2.5rem] bg-slate-50 p-10 border-2 border-slate-100">
                  <div className="flex justify-between items-start mb-8">
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Populasi Mahasiswa & Alumni</p>
                    </div>
                    <div className="text-right">
                      <p className="text-4xl font-black text-emerald-600 leading-none">{currentStats.rate}%</p>
                      <p className="text-[9px] font-black uppercase text-emerald-500 mt-1 tracking-widest text-right">Hired Rate</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase">
                        ...
                        <span className="bg-white px-3 py-1 rounded-lg shadow-sm font-bold tracking-tight">{currentStats.total}</span>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase text-emerald-600">
                        ...
                        <span className="bg-white px-3 py-1 rounded-lg shadow-sm font-bold tracking-tight">{currentStats.hired}</span>
                      </div>
                      <div className="h-4 rounded-full bg-slate-200 overflow-hidden">
               ...
               <div className="rounded-[2.5rem] bg-slate-900 p-10 text-white relative overflow-hidden flex flex-col justify-between text-left">
                  <div className="relative z-10">
                    ...
                    <h4 className="text-2xl font-black italic uppercase tracking-tighter leading-tight">Mulai Audit Inklusi</h4>
                  ...
                  <button 
                    ...
                    className="relative z-10 mt-8 w-full py-4 rounded-2xl bg-white text-slate-900 text-[10px] font-black uppercase tracking-widest hover:bg-emerald-400 transition-all flex items-center justify-center gap-2 font-bold"
                  >
                    Buka Panel Audit <ChevronRight size={16} />
                  </button>
                  <School className="absolute -right-10 -bottom-10 opacity-10" size={200} aria-hidden="true" />
               </div>
            </div>
          </div>
        )}
