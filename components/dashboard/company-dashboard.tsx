...
// Konstanta untuk data terstruktur
...
export default function CompanyDashboard({ user }: { user: any }) {
  ...
  // State Form Lowongan Terstruktur
  ...
  async function postJob() {
    ...
    // Data yang dikirim ke database sekarang sangat rapi & terstruktur
    const { error } = await supabase.from('jobs').insert({
        ...
        target_disabilities: targetDisabilities, // Disimpan sebagai array
        is_active: true
    })
  ...
  if (!company) {
    // ... (Form Profil Perusahaan tetap sama seperti sebelumnya)
    return (
      ...
      {activeTab === "overview" && (
          <div className="space-y-8">
            <div className="grid gap-6 md:grid-cols-3">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm"><Users className="h-8 w-8 text-blue-600 mb-4" /><h3 className="text-xs text-slate-500 font-black uppercase">Pelamar</h3><p className="text-3xl font-black">0</p></div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm"><Briefcase className="h-8 w-8 text-purple-600 mb-4" /><h3 className="text-xs text-slate-500 font-black uppercase">Lowongan</h3><p className="text-3xl font-black">0</p></div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm"><ShieldCheck className="h-8 w-8 text-orange-600 mb-4" /><h3 className="text-xs text-slate-500 font-black uppercase">Audit Status</h3><p className="text-lg font-black text-orange-600">PENDING</p></div>
            </div>
          </div>
      )}
      ...
      {activeTab === "post-job" && (
          <div className="grid lg:grid-cols-5 gap-8 items-start">
            ...
            <div className={`lg:col-span-3 bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm ${isPreview ? 'hidden lg:block opacity-30 pointer-events-none' : 'block'}`}>
                <h3 className="text-xl font-bold mb-8 flex items-center gap-2 text-slate-900 dark:text-slate-100"><Edit3 className="h-5 w-5 text-blue-600" /> Detail Lowongan Inklusif</h3>
                
                <div className="space-y-6">
                    {/* Baris 1: Judul */}
                    <div className="space-y-2">
                        <label className="text-xs font-black uppercase text-slate-500 tracking-widest">Judul Posisi</label>
                        <input type="text" value={jobTitle} onChange={e => setJobTitle(e.target.value)} className="input-std text-lg font-medium" placeholder="Contoh: Staff Administrasi (Disabilitas Netra)" />
                    </div>

                    {/* Baris 2: Lokasi & Mode Kerja */}
                    ...
                    {/* Baris 3: Ragam Disabilitas (Data Terstruktur untuk Riset) */}
                        ...
                        <div className="flex flex-wrap gap-2">
                            {DISABILITY_TYPES.map(type => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${targetDisabilities.includes(type) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500 border-slate-200 hover:border-blue-400'}`}
                                >
                                    {type}
                                </button>
                            ))}
                        </div>
                        <p className="text-[10px] text-slate-400 italic">*Pilihan ini membantu algoritma kami mencocokkan talenta yang tepat.</p>
                    </div>

                    {/* Baris 4: Deskripsi */}
                    <div className="space-y-2">
                        <label className="text-xs font-black uppercase text-slate-500 tracking-widest">Deskripsi & Akomodasi</label>
                ...
                {!isPreview ? (
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center border-b pb-4">
                            ...
                            <button onClick={() => setIsPreview(false)} className="text-[10px] font-black bg-slate-100 px-2 py-1 rounded uppercase">Edit</button>
                        </div>
                        
                        <div className="space-y-4">
                            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200">
                                <h1 className="text-xl font-black mb-1">{jobTitle}</h1>
                                <p className="text-[10px] font-bold text-blue-600 uppercase mb-3 tracking-tighter">{jobWorkMode} • {jobLocation}</p>
                                ...
                                <div className="text-xs text-slate-600 dark:text-slate-300 whitespace-pre-wrap leading-relaxed border-t pt-4">
                                    {jobDesc}
                                </div>
                            </div>

                            <div className="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
                                <h4 className="text-[10px] font-black text-blue-700 uppercase mb-3 flex items-center gap-2">Audit Inklusi Instan</h4>
                                <ul className="space-y-2">
                                    <li className="flex items-center gap-2 text-[10px] font-bold"><CheckCircle2 className={`h-3 w-3 ${jobDesc.toLowerCase().includes('akomodasi') ? 'text-green-600' : 'text-slate-300'}`} /> Informasi Akomodasi</li>
                                    <li className="flex items-center gap-2 text-[10px] font-bold"><CheckCircle2 className={`h-3 w-3 ${targetDisabilities.length > 0 ? 'text-green-600' : 'text-slate-300'}`} /> Target Ragam Terarah</li>
                                </ul>
                            </div>
                            ...
                            <button onClick={postJob} className="w-full h-14 bg-blue-600 text-white rounded-xl font-black text-lg shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-3">
                                <Send className="h-5 w-5" /> PUBLIKASIKAN
                            </button>
