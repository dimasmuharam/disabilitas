    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className={`px-8 py-4 rounded-2xl font-black uppercase text-[10px] flex items-center gap-3 transition-all shadow-xl ${showForm ? 'bg-white border-2 border-slate-900' : 'bg-slate-900 text-white'}`}
        >
      ...
      {showForm ? (
        <form onSubmit={handleSubmit} className="bg-white p-8 md:p-12 rounded-[3rem] border-2 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] space-y-12">
          <fieldset className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-2">
                ...
                <input id="job-title" required value={jobData.title} onChange={e => setJobData({...jobData, title: e.target.value})} className="w-full p-4 rounded-2xl border-2 border-slate-100 font-bold focus:border-blue-600 outline-none" />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Gaji Min</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Gaji Max</label>
          ...
          <fieldset className="space-y-8 p-8 bg-slate-50 rounded-[2.5rem] border-2 border-slate-100">
            <legend className="text-[10px] font-black uppercase text-slate-500 px-4 py-1 bg-white border border-slate-200 rounded-full flex items-center gap-2">2. Kriteria Kompetensi</legend>
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <input value={majorSearch} onChange={(e) => setMajorSearch(e.target.value)} placeholder="Ketik jurusan..." className="w-full p-4 pl-12 rounded-2xl border-2 border-white font-bold outline-none focus:border-blue-600 shadow-sm" />
                {majorSearch && (
                  <div className="absolute z-20 w-full mt-2 bg-white border-2 border-slate-100 rounded-2xl shadow-xl max-h-48 overflow-y-auto p-2">
                    {UNIVERSITY_MAJORS.filter(m => m.toLowerCase().includes(majorSearch.toLowerCase())).map(m => (
                      <button key={m} type="button" onClick={() => toggleTag('required_education_major', m)} className="w-full text-left p-3 hover:bg-blue-50 rounded-xl font-bold text-[10px] uppercase">{m}</button>
                    ))}
                  </div>
                )}
              </div>
              <div className="flex flex-wrap gap-2">
                {jobData.required_education_major.map(m => (
                  <span key={m} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase flex items-center gap-2">
                    {m} <X size={14} className="cursor-pointer" onClick={() => toggleTag('required_education_major', m)} />
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4 pt-6 border-t border-slate-200">
              ...
              <div className="relative">
                ...
                <input value={skillSearch} onChange={(e) => setSkillSearch(e.target.value)} placeholder="Ketik skill..." className="w-full p-4 pl-12 rounded-2xl border-2 border-white font-bold outline-none focus:border-blue-600 shadow-sm" />
                {skillSearch && (
                  <div className="absolute z-20 w-full mt-2 bg-white border-2 border-slate-100 rounded-2xl shadow-xl max-h-48 overflow-y-auto p-2">
                    {SKILLS_LIST.filter(s => s.toLowerCase().includes(skillSearch.toLowerCase())).map(s => (
                      <button key={s} type="button" onClick={() => toggleTag('required_skills', s)} className="w-full text-left p-3 hover:bg-emerald-50 rounded-xl font-bold text-[10px] uppercase">{s}</button>
                    ))}
                  </div>
                )}
              </div>
              <div className="flex flex-wrap gap-2">
                {jobData.required_skills.map(s => (
                  <span key={s} className="px-4 py-2 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase flex items-center gap-2">
                    {s} <X size={14} className="cursor-pointer" onClick={() => toggleTag('required_skills', s)} />
                  </span>
                ))}
              </div>
            </div>
          ...
          <fieldset className="space-y-8">
            <legend className="text-sm font-black uppercase text-emerald-600 pb-4 border-b-2 border-emerald-50 w-full mb-8 flex items-center gap-2">3. Akomodasi & Inklusi</legend>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {ACCOMMODATION_TYPES.map(acc => (
                <label key={acc} className="flex items-center gap-4 p-4 bg-white border-2 border-slate-100 has-[:checked]:border-emerald-600 rounded-2xl cursor-pointer hover:bg-emerald-50 transition-all">
                  <input type="checkbox" checked={jobData.preferred_disability_tools.includes(acc)} onChange={() => toggleTag('preferred_disability_tools', acc)} className="w-5 h-5 rounded border-slate-300 text-emerald-600" />
                  <span className="text-[10px] font-black uppercase text-slate-600">{acc}</span>
                </label>
              ))}
            </div>
            <textarea required rows={6} value={jobData.description} onChange={e => setJobData({...jobData, description: e.target.value})} className="w-full p-6 rounded-[2rem] border-2 border-slate-100 font-medium text-sm outline-none focus:border-blue-600" placeholder="Deskripsi tugas..." />
          </fieldset>

          <button type="submit" disabled={loading} className="w-full py-6 bg-blue-600 text-white rounded-[2.5rem] font-black uppercase text-sm shadow-xl flex items-center justify-center gap-3">
            {loading ? "MEMPROSES..." : (isEditing ? "SIMPAN PERUBAHAN" : "TAYANGKAN LOWONGAN")}
          </button>
        </form>
      ) : (
        <div className="grid gap-4">
          {myJobs.length > 0 ? myJobs.map(job => (
            <div key={job.id} className="bg-white p-8 rounded-[3rem] border-2 border-slate-100 flex justify-between items-center group hover:border-slate-900 transition-all gap-6">
              <div className="flex items-center gap-6">
                <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-blue-600 transition-all"><Briefcase size={28} /></div>
                <div className="text-left">
                  <h4 className="font-black text-slate-900 uppercase text-sm">{job.title}</h4>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{job.location} • {job.job_type}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => handleEdit(job)} className="p-4 bg-slate-50 text-slate-400 hover:bg-blue-600 hover:text-white rounded-2xl transition-all shadow-sm"><Edit3 size={20} /></button>
                <button onClick={() => handleDelete(job.id, job.title)} className="p-4 bg-slate-50 text-slate-400 hover:bg-red-600 hover:text-white rounded-2xl transition-all shadow-sm"><Trash2 size={20} /></button>
              </div>
            </div>
          )) : (
            <div className="p-20 border-4 border-dashed border-slate-50 rounded-[4rem] text-center opacity-50">
              <p className="text-[10px] font-black text-slate-300 uppercase italic">Belum ada lowongan.</p>
            </div>
          )}
        </div>
      )}
