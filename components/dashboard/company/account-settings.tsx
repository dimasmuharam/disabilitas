...
export default function AccountSettings({ user, onSuccess }: { user: any, onSuccess: () => void }) {
  ...
  const handleUpdateEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ text: `{"Cek inbox email baru Anda."}`, type: "success" });
    } catch (err: any) {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      announce(`{"Sandi berhasil diperbarui."}`);
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4">
      ...
      <section className="bg-white p-10 rounded-[3rem] border-2 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] space-y-10">
        <div className="flex items-center gap-4 border-b pb-6">
          ...
          <div>
            <h2 className="text-xl font-black uppercase italic tracking-tighter">{"Akses & Keamanan"}</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{"Kredensial Login Instansi"}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleUpdateEmail} className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <button type="submit" disabled={loading || email === user?.email} className="px-6 py-2 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase disabled:opacity-50 transition-all">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            <button type="button" onClick={handleLogoutAll} disabled={loading} className="px-6 py-3 bg-slate-100 text-slate-700 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-slate-200 transition-all">
        ...
        <div className="p-8 bg-red-50 rounded-[2rem] border-2 border-red-100 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-[10px] font-bold text-red-700 uppercase tracking-tight leading-relaxed max-w-xs">
            {"Menghapus akun akan menghapus seluruh data instansi secara permanen."}
          </div>
          <button type="button" onClick={handleDeleteAccount} className="px-8 py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg shadow-red-100">
            {"Hapus Akun"}
          </button>
        </div>
      </section>
      ...
      {message.text && (
        <div className={`p-4 rounded-2xl flex items-center gap-3 border-2 ${message.type === "success" ? "bg-emerald-50 border-emerald-100 text-emerald-700" : "bg-red-50 border-red-100 text-red-700"}`} role="alert">
          ...
          <p className="text-[10px] font-black uppercase leading-none">{message.text}</p>
        </div>
      )}
    </div>
  );
