  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 font-sans text-slate-900">
      ...
      <form onSubmit={handleUpdateAccount} className="space-y-10 px-4">
        ...
        <section className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-xl">
          <div className="text-xs font-black uppercase tracking-widest text-blue-400">{"Logout Global"}</div>
          <button type="button" onClick={handleLogoutGlobal} className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase transition-all active:scale-95">
            {"Logout Semua Perangkat"}
          </button>
        </section>

        <div className="flex justify-end gap-4 pt-6">
          <button type="button" onClick={onSuccess} className="px-10 py-5 rounded-[2rem] font-black uppercase text-sm text-slate-500 border-2 border-slate-100 hover:bg-slate-50 transition-all">
            {"Kembali"}
          </button>
