...
export default function ApplicantTracker({ company }: { company: any }) {
  ...
  const renderSmartButtons = (app: any) => {
    ...
    if (status === 'hired') return <div className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-center text-[10px] font-black italic text-emerald-600">TALENTA SUDAH BEKERJA</div>;
    if (status === 'rejected') return <div className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-center text-[10px] font-black italic text-red-600">LAMARAN DITOLAK</div>;
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        {status === 'applied' && (
          <button 
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-[9px] font-black uppercase text-white transition-all hover:bg-slate-900"
          >
        ...
        {status === 'reviewing' && (
          <button 
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-orange-500 px-4 py-3 text-[9px] font-black uppercase text-white transition-all hover:bg-slate-900"
          >
        ...
        {status === 'interview' && (
          <button 
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-[9px] font-black uppercase text-white transition-all hover:bg-slate-900"
          >
        ...
        {status === 'accepted' && (
          <button 
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-[9px] font-black uppercase text-white transition-all hover:bg-slate-900"
          >
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 rounded-xl border-2 border-slate-100 bg-white px-4 py-2 text-[9px] font-black uppercase text-red-500 transition-all hover:border-red-200 hover:bg-red-50"
        >
  ...
  if (loading) return <div role="status" className="animate-pulse p-20 text-center font-black uppercase italic text-slate-400">Sinkronisasi Pusat Riset...</div>;
    ...
    <div className="mx-auto max-w-6xl space-y-8 pb-32 text-left">
      ...
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        ...
        <button 
          ...
          className="rounded-2xl bg-slate-900 px-8 py-4 text-[10px] font-black uppercase italic text-white shadow-lg transition-all hover:bg-blue-600"
        >
      ...
      <div className="grid gap-4 rounded-[2.5rem] border-2 border-slate-100 bg-white p-6 md:grid-cols-3">
        <input 
          ...
          className="rounded-xl border-2 border-slate-50 bg-slate-50 px-6 py-3 text-[10px] font-black outline-none transition-all focus:border-slate-900"
          ...
          return (
            <article key={app.id} className={`flex flex-col items-start gap-6 rounded-[3rem] border-2 p-8 transition-all md:flex-row md:items-center ${selectedIds.includes(app.id) ? 'border-blue-600 bg-blue-50/50' : 'border-slate-100 bg-white hover:border-slate-900'}`}>
              ...
              <div className="relative shrink-0">
                ...
                />
                <label htmlFor={`check-${app.id}`} className="cursor-pointer text-slate-200 transition-colors peer-checked:text-blue-600" aria-label={`Pilih ${p?.full_name}`}>
              ...
              <div className="flex-1 space-y-1">
                ...
                <p className="text-[10px] font-black uppercase italic tracking-[0.2em] text-slate-400">Posisi: {app.jobs?.title}</p>
                
                {/* Deskripsi Lengkap Sesuai Permintaan */}
                <div className="mt-4 rounded-2xl border border-slate-100 bg-slate-50 p-6 text-sm font-medium italic leading-relaxed text-slate-700 shadow-sm">
                  ...
                  {skills.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {skills.slice(0, 5).map((s: string, i: number) => (
                        <span key={i} className="rounded-lg border border-slate-200 bg-white px-3 py-1 text-[8px] font-bold uppercase not-italic text-slate-500 shadow-sm">
                          {s}
                        </span>
                      ))}
                    </div>
              ...
              <div className="flex w-full flex-col gap-3 md:w-64">
                ...
                <div className="mt-2 flex gap-2">
                  <button onClick={() => generateProfessionalCV(app, company)} title="Cetak CV Audit" className="flex-1 rounded-xl border border-slate-100 bg-slate-50 p-3 text-slate-400 transition-all hover:text-blue-600"><FileDown size={22} className="mx-auto" /></button>
                  <a href={`/talent/${p?.id}`} target="_blank" title="Buka Profil Lengkap" className="flex-1 rounded-xl border border-slate-100 bg-slate-50 p-3 text-slate-400 transition-all hover:text-slate-900"><ExternalLink size={22} className="mx-auto" /></a>
                </div>
              </div>
      ...
      {noteModal.isOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-[3rem] border-4 border-slate-900 bg-white p-10 shadow-2xl duration-200 animate-in zoom-in-95">
            ...
            <textarea 
              autoFocus 
              className="mt-6 h-32 w-full rounded-2xl border-2 border-slate-100 bg-slate-50 p-5 text-sm font-medium italic outline-none transition-all focus:border-blue-600" 
              placeholder="Catatan riset internal (Opsional)..." 
              value={hrdNote} 
              onChange={(e) => setHrdNote(e.target.value)} 
            />
            <div className="mt-6 flex gap-3">
              <button onClick={() => setNoteModal({ ...noteModal, isOpen: false })} className="flex-1 rounded-xl border-2 border-slate-100 py-4 text-[10px] font-black uppercase transition-all hover:bg-slate-50">BATAL</button>
              <button onClick={() => handleUpdateStatus(noteModal.appId, noteModal.status, hrdNote)} className="flex-1 rounded-xl bg-slate-900 py-4 text-[10px] font-black uppercase italic text-white shadow-xl transition-all hover:bg-blue-600">KONFIRMASI</button>
            </div>
          </div>
        </div>
      )}
