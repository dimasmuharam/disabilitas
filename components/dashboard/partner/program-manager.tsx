...
export default function ProgramManager({ partnerId, onBack }: ProgramManagerProps) {
  ...
  // LOGIKA INTERNAL SYSTEM GENERATE SLUG
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setStatusMsg({ text: "Mengamankan data ke sistem...", isError: false });

    try {
      // Slug otomatis dibuat di sini jika data baru
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 gap-12 lg:grid-cols-3 text-left">
        <div className="lg:col-span-2 space-y-10">
          <section className="rounded-[3rem] border-4 border-slate-900 bg-white p-10 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)] space-y-8">
            <h2 className="flex items-center gap-3 text-2xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">
              <Zap className="text-blue-600" /> Informasi Kegiatan
            </h2>
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                ...
                {formData.slug && (
                  <p className="ml-2 text-[8px] font-black uppercase text-blue-500 italic flex items-center gap-1">
                    <Link2 size={10}/> URL: disabilitas.com/pelatihan/{formData.slug}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <label htmlFor="loc-select" className="ml-1 text-[10px] font-black uppercase text-slate-400">Kota Lokasi</label>
                  {deliveryMethod === "Online" ? (
                    ...
                    <div className="relative">
                      <select id="loc-select" value={formData.location} onChange={e => e.target.value === "LAINNYA" ? setIsCustomCity(true) : setFormData({...formData, location: e.target.value})} className="w-full appearance-none rounded-2xl border-2 border-slate-100 bg-slate-50 p-4 font-bold outline-none focus:border-slate-900">
                    ...
                    <div className="space-y-2">
                      <input ref={manualCityRef} value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="w-full rounded-2xl border-2 border-blue-100 bg-blue-50 p-4 font-bold outline-none" placeholder="Ketik Kota..." />
          ...
          <section className="rounded-[3rem] border-4 border-slate-900 bg-white p-10 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)] space-y-8">
            <h2 className="flex items-center gap-3 text-2xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">
              <BookOpen className="text-blue-600" /> Kurikulum
            </h2>
            <div className="space-y-6">
              <div className="space-y-2">
                <label htmlFor="desc" className="ml-1 text-[10px] font-black uppercase text-slate-400">Deskripsi</label>
                <textarea id="desc" rows={3} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full rounded-3xl border-2 border-slate-100 bg-slate-50 p-6 font-medium outline-none focus:border-slate-900" />
              </div>
              <div className="space-y-2">
                <label htmlFor="syllabus" className="ml-1 text-[10px] font-black uppercase text-slate-400">Agenda Sesi</label>
                <textarea id="syllabus" rows={3} value={formData.syllabus} onChange={e => setFormData({...formData, syllabus: e.target.value})} className="w-full rounded-3xl border-2 border-slate-100 bg-slate-50 p-6 font-medium outline-none focus:border-slate-900" />
              </div>
            </div>
          </section>
          ...
          <section className="rounded-[3rem] border-4 border-slate-900 bg-blue-600 p-10 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)] text-white space-y-6">
            <h2 className="text-2xl font-black uppercase italic tracking-tighter leading-none text-left">Instruksi Otomatis</h2>
            <textarea id="reg_instructions" rows={3} required value={formData.registration_instructions} onChange={e => setFormData({...formData, registration_instructions: e.target.value})} className="w-full rounded-2xl border-2 border-blue-400 bg-blue-700 p-6 font-bold text-white placeholder:text-blue-300 outline-none focus:border-white" placeholder="Link grup WA / Zoom akan muncul setelah talenta klik DAFTAR." />
          </section>
        </div>

        <div className="space-y-8">
          <fieldset className="rounded-[2.5rem] bg-slate-900 p-8 text-white shadow-2xl text-left">
            <h3 className="mb-6 text-[11px] font-black uppercase tracking-widest italic text-blue-400 flex items-center gap-2"><Users size={18}/> Ragam Sasaran</h3>
            <div className="space-y-3">
              {DISABILITY_TYPES.map((type) => (
                <label key={type} className={`flex cursor-pointer items-center justify-between rounded-xl border-2 p-4 transition-all ${formData.target_disability.includes(type) ? 'border-blue-500 bg-blue-600/20' : 'border-slate-800 bg-slate-800/50 hover:border-slate-700'}`}>
                  <input type="checkbox" checked={formData.target_disability.includes(type)} onChange={() => handleMultiToggle("target_disability", type)} className="size-5 accent-blue-500" />
          ...
          <fieldset className="rounded-[2.5rem] bg-slate-900 p-8 text-white shadow-2xl text-left">
            <h3 className="mb-6 text-[11px] font-black uppercase tracking-widest italic text-emerald-400 flex items-center gap-2"><ListChecks size={18}/> Akomodasi</h3>
            <div className="space-y-3">
              {ACCOMMODATION_TYPES.map((acc) => (
                <label key={acc} className={`flex cursor-pointer items-center justify-between rounded-xl border-2 p-4 transition-all ${formData.training_accommodations.includes(acc) ? 'border-emerald-500 bg-emerald-600/20' : 'border-slate-800 bg-slate-800/50 hover:border-slate-700'}`}>
                  <input type="checkbox" checked={formData.training_accommodations.includes(acc)} onChange={() => handleMultiToggle("training_accommodations", acc)} className="size-5 accent-emerald-500" />
          ...
          <div className="space-y-4">
            ...
            {statusMsg.text && (
              <div className={`flex items-center gap-3 rounded-2xl border-2 p-5 text-[10px] font-black uppercase ${statusMsg.isError ? 'border-red-100 bg-red-50 text-red-600' : 'border-emerald-100 bg-emerald-50 text-emerald-700'}`}>
            ...
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-3 rounded-[2rem] bg-slate-900 py-7 text-xs font-black uppercase italic tracking-[0.2em] text-white shadow-2xl transition-all hover:bg-blue-600 disabled:opacity-50 active:scale-95">
              {loading ? "MENYELARASKAN..." : <><Save size={20} /> SIMPAN PROGRAM</>}
            </button>
          </div>
        </div>
      </form>
        ...
        {programs.map(prog => (
          <div key={prog.id} className="group relative flex flex-col justify-between rounded-[3rem] border-4 border-slate-900 bg-white p-8 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)] transition-all hover:shadow-[12px_12px_0px_0px_rgba(37,99,235,1)]">
            <div className="space-y-4 text-left">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                   <button onClick={() => handleEdit(prog)} className="text-slate-300 hover:text-blue-600 transition-all"><BookOpen size={16} /></button>
                   <button onClick={async () => { if(confirm(`Hapus program ${prog.title}?`)) { await supabase.from("trainings").delete().eq("id", prog.id); fetchPrograms(); } }} className="text-slate-300 hover:text-red-600 transition-all"><Trash2 size={16} /></button>
                </div>
              </div>
            ...
            <div className="mt-8 flex items-center justify-between border-t-2 border-slate-50 pt-6">
              ...
              <button onClick={() => handleEdit(prog)} className="text-[10px] font-black uppercase italic text-blue-600 underline underline-offset-4">Edit</button>
            </div>
          </div>
        ))}
      </div>
