...
export default function GovProfileEditor({ user, company, onSaveSuccess }: GovProfileEditorProps) {
  ...
  const handleSearchRegion = (query: string) => {
    ...
    // Mengacu ke data-static / constants
    if (isProvCategory) {
    ...
    // Voice Feedback untuk Screen Reader
    if (results.length > 0) {
      setAnnouncement(`Ditemukan ${results.length} wilayah. Gunakan panah bawah untuk memilih.`);
    } else {
      setAnnouncement("Wilayah tidak ditemukan.");
    }
  };
  ...
  const selectRegion = (region: any) => {
    ...
    setRegionResults([]);
    setActiveIndex(-1);
    setAnnouncement(`Wilayah ${region.name} terpilih.`);
    inputRef.current?.focus();
  };

  // Navigasi Keyboard (Accessibility Fix)
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setActiveIndex(prev => (prev < regionResults.length - 1 ? prev + 1 : prev));
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      setActiveIndex(prev => (prev > 0 ? prev - 1 : -1));
    } else if (e.key === "Enter" && activeIndex >= 0) {
      e.preventDefault();
      selectRegion(regionResults[activeIndex]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const { error } = await supabase.from("government").upsert({
        id: user.id,
        ...formData,
  ...
  const handleRequestVerification = async () => {
    if (!formData.verification_document_link || !formData.verification_document_link.trim()) {
      setAnnouncement("Kesalahan: Dokumen verifikasi kosong.");
      return;
    }
    if (!formData.verification_document_link.includes("drive.google.com")) {
      setAnnouncement("Kesalahan: Link dokumen tidak valid.");
      return;
    }
    setLoading(true);
      ...
      <div className="grid gap-10">
          ...
          <section className="rounded-[3rem] border-4 border-dashed border-blue-600 bg-blue-50 p-10 shadow-xl" aria-labelledby="section-verif">
            <div className="mb-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-blue-500">Unggah berkas resmi ke Google Drive</p>
              </div>
            </div>
        ...
        <section className="rounded-[2.5rem] border-4 border-slate-900 bg-white p-8 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)]" aria-labelledby="section-otoritas">
          <div className="mb-8 flex items-center gap-4 border-b-2 border-slate-100 pb-4">
            ...
            <div>
              <h2 id="section-otoritas" className="text-xl font-black uppercase italic tracking-tight text-slate-900">Cakupan Wilayah Kerja</h2>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Identitas administratif otoritas</p>
            </div>
          </div>
          ...
          <div className="grid gap-8 md:grid-cols-2">
            <div className="space-y-2">
              ...
              >
                {GOV_INSTANSI_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
            ...
            {isNasional ? (
              <div className="space-y-2">
                ...
                <input id="manual-name" type="text" placeholder="Contoh: Kementerian Sosial RI" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full rounded-2xl border-4 border-slate-900 p-4 font-bold outline-none focus:ring-4 focus:ring-blue-100" />
              </div>
            ) : (
              <div className="relative space-y-2" ref={searchRef}>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    aria-activedescendant={activeIndex >= 0 ? `region-opt-${activeIndex}` : undefined}
                ...
                {regionResults.length > 0 && (
                  <ul 
                    ...
                    role="listbox" 
                    className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border-4 border-slate-900 bg-white shadow-2xl"
                  >
                    {regionResults.map((region, idx) => (
                      <li 
                        ...
                        className={`group flex cursor-pointer items-center justify-between p-4 transition-colors ${activeIndex === idx ? "bg-slate-900 text-white" : "hover:bg-slate-50"}`}
                      >
                        <span className="font-black uppercase italic">{region.name}</span>
                        <span className={`text-[10px] font-bold ${activeIndex === idx ? "text-blue-400" : "text-slate-400"}`}>{region.type}</span>
                      </li>
                    ))}
                  </ul>
                )}
        ...
        <section className="rounded-[2.5rem] border-4 border-slate-900 bg-white p-8 shadow-[12px_12px_0px_0px_rgba(15,23,42,1)]" aria-labelledby="section-kontak">
          <div className="mb-8 flex items-center gap-4 border-b-2 border-slate-100 pb-4">
            ...
            <h2 id="section-kontak" className="text-xl font-black uppercase italic tracking-tight text-slate-900">Atribut Operasional</h2>
          </div>
          <div className="grid gap-8 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input id="wa" type="text" value={formData.whatsapp_official || ""} onChange={e => setFormData({...formData, whatsapp_official: e.target.value})} placeholder="628123..." className="w-full rounded-2xl border-4 border-slate-900 p-4 pl-12 font-bold outline-none focus:ring-4 focus:ring-emerald-50" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <input id="seal" type="url" value={formData.official_seal_url || ""} onChange={e => setFormData({...formData, official_seal_url: e.target.value})} placeholder="https://..." className="w-full rounded-2xl border-4 border-slate-900 p-4 font-bold outline-none focus:ring-4 focus:ring-emerald-50" />
            </div>
          </div>
          <div className="mt-8 space-y-2">
            ...
            <textarea id="desc" value={formData.description || ""} onChange={e => setFormData({...formData, description: e.target.value})} rows={4} className="w-full rounded-2xl border-4 border-slate-900 p-4 font-bold outline-none focus:ring-4 focus:ring-emerald-50" placeholder="Visi misi inklusi instansi..." />
          </div>
        </section>
        ...
        <div className="space-y-6 border-t border-slate-100 pt-10">
          ...
          <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
            <div className="flex flex-wrap gap-4">
              <button type="button" onClick={handleSaveProfile} disabled={loading} className="flex items-center gap-4 rounded-3xl border-4 border-slate-900 bg-white px-10 py-5 text-[11px] font-black uppercase tracking-widest text-slate-900 hover:bg-slate-50 disabled:opacity-50">
                {loading && !announcement.includes("verifikasi") ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
                Simpan Profil Otoritas
              </button>
              {!company?.is_verified && (
                <button type="button" onClick={handleRequestVerification} disabled={loading} className="flex items-center gap-4 rounded-3xl bg-blue-600 px-10 py-5 text-[11px] font-black uppercase tracking-widest text-white shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] hover:bg-blue-700 disabled:opacity-50">
