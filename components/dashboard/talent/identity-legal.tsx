...
// MENGGUNAKAN VARIABEL YANG BENAR DARI DATA-STATIC
import { CITIES, DISABILITY_TYPES } from "@/lib/data-static";
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                ...
                <input id="full_name" type="text" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.full_name} onChange={(e) => setFormData({...formData, full_name: e.target.value})} />
              </div>
              <div>
                <label htmlFor="birth_date" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Tanggal Lahir"}</label>
                <input id="birth_date" type="date" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.birth_date} onChange={(e) => setFormData({...formData, birth_date: e.target.value})} />
              </div>
              <div>
                <label htmlFor="gender" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Jenis Kelamin"}</label>
                <select id="gender" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.gender} onChange={(e) => setFormData({...formData, gender: e.target.value})}>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input id="phone_number" type="text" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.phone_number} onChange={(e) => setFormData({...formData, phone_number: e.target.value})} />
              </div>
              <div>
                <label htmlFor="city" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Kota Domisili"}</label>
                <input id="city" list="city-options" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} />
              ...
              <div>
                ...
                <select id="comm_pref" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.communication_preference} onChange={(e) => setFormData({...formData, communication_preference: e.target.value})}>
        ...
        <section className="bg-white p-10 rounded-[3rem] border-2 border-slate-100 shadow-sm space-y-8" aria-labelledby="digital-docs-title">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                ...
                <input id="portfolio" type="url" placeholder="https://..." className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-purple-600 transition-all text-sm" value={formData.portfolio_url} onChange={(e) => setFormData({...formData, portfolio_url: e.target.value})} />
              </div>
              <div>
                <label htmlFor="resume" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Link Resume (Google Drive)"}</label>
                <input id="resume" type="url" placeholder="https://drive.google.com/..." className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-purple-600 transition-all text-sm" value={formData.resume_url} onChange={(e) => setFormData({...formData, resume_url: e.target.value})} />
                {driveWarning && (
                  <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 mt-2 flex items-start gap-3 animate-in slide-in-from-top-2" role="alert">
                    <Info size={16} className="text-amber-600 mt-0.5" />
                    <p className="text-[9px] font-bold text-amber-700 uppercase leading-tight">
                      {"Pastikan link Google Drive sudah diatur ke 'Anyone with the link' agar HRD bisa melihat CV Anda."}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input id="dis_proof" type="url" placeholder="Link dokumen bukti" className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-purple-600 transition-all text-sm" value={formData.disability_proof_url} onChange={(e) => setFormData({...formData, disability_proof_url: e.target.value})} />
              </div>
              <div>
                <label htmlFor="youtube" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Link Video YouTube"}</label>
                <input id="youtube" type="url" placeholder="https://youtube.com/..." className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-purple-600 transition-all text-sm" value={formData.intro_video_url} onChange={(e) => setFormData({...formData, intro_video_url: e.target.value})} />
              </div>
            </div>
          </div>
          <div className="bg-slate-50 p-6 rounded-2xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase leading-relaxed italic">
              {"Buat video singkat 1 menit perkenalan diri dan unggah ke YouTube (Unlisted). Fitur ini sangat membantu HRD mengenal Anda."}
            </p>
          </div>
        </section>

        {/* SEKSI 3: BIO & PERSETUJUAN (WAJIB) */}
        <section className="bg-white p-10 rounded-[3rem] border-2 border-slate-100 shadow-sm space-y-6">
          <div className="space-y-2">
            <label htmlFor="disability_type" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Ragam Disabilitas (Wajib)"}</label>
            <select id="disability_type" required className="w-full bg-slate-50 border-2 border-slate-50 p-4 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all text-sm" value={formData.disability_type} onChange={(e) => setFormData({...formData, disability_type: e.target.value})}>
              <option value="">{"Pilih Ragam"}</option>
              {DISABILITY_TYPES.map((type: string) => <option key={type} value={type}>{type}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="bio" className="text-[10px] font-bold uppercase ml-2 text-slate-400">{"Bio Singkat (Wajib)"}</label>
            <textarea id="bio" required className="w-full bg-slate-50 border-2 border-slate-50 p-6 rounded-[2rem] font-bold outline-none focus:border-blue-600 h-32 italic transition-all text-sm" placeholder="Tuliskan siapa Anda..." value={formData.bio} onChange={(e) => setFormData({...formData, bio: e.target.value})} />
          </div>
          <label className="flex items-center gap-4 p-5 bg-blue-50 border-2 border-blue-100 rounded-3xl cursor-pointer hover:bg-blue-100 transition-all shadow-sm">
